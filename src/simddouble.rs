...
#[inline]
fn VOpMask vnot_vo64_vo64(VOpMask x) {
  return vxor_vo_vo_vo(x, veq64_vo_vm_vm(vcast_vm_i_i(0, 0), vcast_vm_i_i(0, 0)));
}
#[inline]
fn vsignbit_vo_vd(d: VDouble) -> VOpMask {
  return veq64_vo_vm_vm(vand_vm_vm_vm(vreinterpret_vm_vd(d), vreinterpret_vm_vd((-0.).as_vd())), vreinterpret_vm_vd((-0.).as_vd()));
}

// return d0 < d1 ? x : y#[inline]
fn vsel_vi_vd_vd_vi_vi(VDouble d0, VDouble d1, VInt x, VInt y) -> VInt { return vsel_vi_vo_vi_vi(vcast_vo32_vo64(vlt_vo_vd_vd(d0, d1)), x, y); } 

// return d0 < 0 ? x : 0#[inline]
fn vsel_vi_vd_vi(VDouble d, VInt x) -> VInt { return vand_vi_vo_vi(vcast_vo32_vo64(vsignbit_vo_vd(d)), x); }
#[inline]
fn visnegzero_vo_vd(d: VDouble) -> VOpMask {
  return veq64_vo_vm_vm(vreinterpret_vm_vd(d), vreinterpret_vm_vd((-0.).as_vd()));
}
#[inline]
fn visnumber_vo_vd(x: VDouble) -> VOpMask {
  return vandnot_vo_vo_vo(visinf_vo_vd(x), veq_vo_vd_vd(x, x));
}
#[inline]
fn vsignbit_vm_vd(d: VDouble) -> vmask {
  return vand_vm_vm_vm(vreinterpret_vm_vd(d), vreinterpret_vm_vd((-0.).as_vd()));
}
#[inline]
fn vmulsign_vd_vd_vd(x: VDouble, y: VDouble) -> VDouble {
  return vreinterpret_vd_vm(vxor_vm_vm_vm(vreinterpret_vm_vd(x), vsignbit_vm_vd(y)));
}
#[inline]
fn vcopysign_vd_vd_vd(x: VDouble, y: VDouble) -> VDouble {
...
#[inline]
fn vsign_vd_vd(d: VDouble) -> VDouble {
  return vmulsign_vd_vd_vd((1.).as_vd(), d);
}
#[inline]
fn vpow2i_vd_vi(q: VInt) -> VDouble {
  q = (0x3ff).as_vi() + q;
  let r = q.as_vi2();
  return vreinterpret_vd_vi2(vsll_vi2_vi2_i(r, 20));
}
#[inline]
fn vldexp_vd_vd_vi(x: VDouble, q: VInt) -> VDouble {
...
#[inline]
fn vldexp2_vd_vd_vi(d: VDouble, e: VInt) -> VDouble {
  return vmul_vd_vd_vd(vmul_vd_vd_vd(d, vpow2i_vd_vi(vsra_vi_vi_i(e, 1))), vpow2i_vd_vi(e - vsra_vi_vi_i(e, 1)));
}
#[inline]
fn vldexp3_vd_vd_vi(d: VDouble, q: VInt) -> VDouble {
...
fn vilogbk_vi_vd(d: VDouble) -> VInt {
  VOpMask o = vlt_vo_vd_vd(d, (4.9090934652977266e-91).as_vd());
  d = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd((2.037035976334486e90).as_vd(), d), d);
...
#[inline]
fn vilogb2k_vi_vd(d: VDouble) -> VInt {
  let q = vreinterpret_vi2_vd(d).as_vi();
  q = vsrl_vi_vi_i(q, 20);
...
#endif
#[inline]
fn visint_vo_vd(d: VDouble) -> VOpMask {
  VDouble x = vtruncate_vd_vd(vmul_vd_vd_vd(d, (1. / D1_31).as_vd()));
  x = (-D1_31).as_vd().mla(x, d);
  return vor_vo_vo_vo(veq_vo_vd_vd(vtruncate_vd_vd(x), x),
		      vgt_vo_vd_vd(vabs_vd_vd(d), D1_53.as_vd()));
}
#[inline]
fn visodd_vo_vd(d: VDouble) -> VOpMask {
  VDouble x = vtruncate_vd_vd(vmul_vd_vd_vd(d, (1. / D1_31).as_vd()));
  x = (-D1_31).as_vd().mla(x, d);

  return vand_vo_vo_vo(vcast_vo64_vo32(veq_vo_vi_vi(vand_vi_vi_vi(vtruncate_vi_vd(x), (1).as_vi()), (1).as_vi())),
		       vlt_vo_vd_vd(vabs_vd_vd(d), (D1_53).as_vd()));
}

//

pub fn xldexp(x: VDouble, q: VInt) -> VDouble { return vldexp_vd_vd_vi(x, q); }

pub fn xilogb(d: VDouble) -> VInt {
  VDouble e = vcast_vd_vi(vilogbk_vi_vd(vabs_vd_vd(d)));
  e = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.).as_vd()), SLEEF_FP_ILOGB0.as_vd(), e);
...
#[inline]
fn rempisub(x: VDouble) -> (VDouble, VInt) {
   if cfg!(feature="full_fp_rounding") {
    VDouble y = vrint_vd_vd(vmul_vd_vd_vd(x, (4.).as_vd()));
    VInt vi = vtruncate_vi_vd(vsub_vd_vd_vd(y, vmul_vd_vd_vd(vrint_vd_vd(x), (4.).as_vd())));
    ( vsub_vd_vd_vd(x, vmul_vd_vd_vd(y, (0.25).as_vd())), vi )
   } else {
    VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_28.as_vd(), vtruncate_vd_vd(vmul_vd_vd_vd(x, (1. / D1_28).as_vd()))));
    VInt vi = vsel_vi_vo_vi_vi(vcast_vo32_vo64(vgt_vo_vd_vd(x, (0.).as_vd())), (4).as_vi(), (3).as_vi()) + vtruncate_vi_vd(vmul_vd_vd_vd(fr, (8.).as_vd()));
    vi = vsra_vi_vi_i(vand_vi_vi_vi((7).as_vi(), vi) - (3).as_vi(), 1);
    fr = vsub_vd_vd_vd(fr, vmul_vd_vd_vd((0.25).as_vd(), vtruncate_vd_vd(fr.mla((4.).as_vd(), vmulsign_vd_vd_vd((0.5).as_vd(), x)))));
    fr = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(vabs_vd_vd(fr), (0.25).as_vd()), vsub_vd_vd_vd(fr, vmulsign_vd_vd_vd((0.5).as_vd(), x)), fr);
    fr = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(vabs_vd_vd(fr), (1e+10).as_vd()), (0).as_vd(), fr);
    VOpMask o = veq_vo_vd_vd(vabs_vd_vd(x), (0.12499999999999998612).as_vd());
...
#[inline]
fn rempi(a: VDouble) -> (VDouble2, VInt) {
  VDouble2 x, y, z;
  ...
  VOpMask o = vlt_vo_vd_vd(vabs_vd_vd(a), (0.7).as_vd());
  x.x = vsel_vd_vo_vd_vd(o, a, x.x);
  x.y = vreinterpret_vd_vm(vandnot_vm_vo64_vm(o, vreinterpret_vm_vd(x.y)));
  ( x, q )
}

pub fn xsin(d: VDouble) -> VDouble {
  VDouble u, s, r = d;
  VInt ql;

  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX2.as_vd())))) {
    VDouble dql = vrint_vd_vd(vmul_vd_vd_vd(d, M_1_PI.as_vd()));
    ql = vrint_vi_vd(dql);
    d = dql.mla((-PI_A2).as_vd(), d);
    d = dql.mla((-PI_B2).as_vd(), d);
  } else if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX.as_vd())))) {
    VDouble dqh = vtruncate_vd_vd(vmul_vd_vd_vd(d, (M_1_PI / D1_24).as_vd()));
    dqh = vmul_vd_vd_vd(dqh, D1_24.as_vd());
    VDouble dql = vrint_vd_vd(vmlapn_vd_vd_vd_vd(d, M_1_PI.as_vd(), dqh));
    ql = vrint_vi_vd(dql);
    ...
    d = vadd_vd_vd_vd(dqh, dql).mla((-PI_D).as_vd(), d);
  } else {
    let (ddidd, ddii) = rempi(d);
    ql = vand_vi_vi_vi(ddii, (3).as_vi());
    ql = ql + ql + vsel_vi_vo_vi_vi(vcast_vo32_vo64(vgt_vo_vd_vd(ddidd.x, (0.).as_vd())), (2).as_vi(), (1).as_vi());
    ql = vsra_vi_vi_i(ql, 2);
    VOpMask o = veq_vo_vi_vi(vand_vi_vi_vi(ddii, (1).as_vi()), (1).as_vi());
    VDouble2 x = vcast_vd2_vd_vd(vmulsign_vd_vd_vd((-3.141592653589793116 * 0.5).as_vd(), ddidd.x), 
				 vmulsign_vd_vd_vd((-1.2246467991473532072e-16 * 0.5).as_vd(), ddidd.x));
    x = ddadd2_vd2_vd2_vd2(ddidd, x);
    ddidd = vsel_vd2_vo_vd2_vd2(vcast_vo64_vo32(o), x, ddidd);
    d = vadd_vd_vd_vd(ddidd.x, ddidd.y);
...
pub fn xsin_u1(d: VDouble) -> VDouble {
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX2).as_vd())))) {
    const VDouble dql = vrint_vd_vd(vmul_vd_vd_vd(d, (M_1_PI).as_vd()));
    ql = vrint_vi_vd(dql);
    u = dql.mla((-PI_A2).as_vd(), d);
    s = ddadd_vd2_vd_vd (u, vmul_vd_vd_vd(dql, (-PI_B2).as_vd()));
  } else if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX).as_vd())))) {
    VDouble dqh = vtruncate_vd_vd(vmul_vd_vd_vd(d, (M_1_PI / D1_24).as_vd()));
    dqh = vmul_vd_vd_vd(dqh, (D1_24).as_vd());
    const VDouble dql = vrint_vd_vd(vmlapn_vd_vd_vd_vd(d, (M_1_PI).as_vd(), dqh));
    ql = vrint_vi_vd(dql);

    u = dqh.mla((-PI_A).as_vd(), d);
    s = ddadd_vd2_vd_vd  (u, vmul_vd_vd_vd(dql, (-PI_A).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dqh, (-PI_B).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_B).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dqh, (-PI_C).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_C).as_vd()));
    s = ddadd_vd2_vd2_vd(s, vmul_vd_vd_vd(vadd_vd_vd_vd(dqh, dql), (-PI_D).as_vd()));
  } else {
    let (ddidd, ddii) = rempi(d);
    ql = vand_vi_vi_vi(ddii, (3).as_vi());
    ql = ql + ql + vsel_vi_vo_vi_vi(vcast_vo32_vo64(vgt_vo_vd_vd(ddidd.x, (0.).as_vd())), (2).as_vi(), (1).as_vi());
  ...
  u = (2.72052416138529567917983e-15).as_vd()
      .mla(s.x, (-7.6429259411395447190023e-13).as_vd())
      .mla(s.x, (1.60589370117277896211623e-10).as_vd())
      .mla(s.x, (-2.5052106814843123359368e-08).as_vd())
      .mla(s.x, (2.75573192104428224777379e-06).as_vd())
      .mla(s.x, (-0.000198412698412046454654947).as_vd())
      .mla(s.x, (0.00833333333333318056201922).as_vd());

  x = ddadd_vd2_vd_vd2((1.).as_vd(), ddmul_vd2_vd2_vd2(ddadd_vd2_vd_vd((-0.166666666666666657414808).as_vd(), vmul_vd_vd_vd(u, s.x)), s));
  ...
  u = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.).as_vd()), d, u);
  
  return u;
}

pub fn xcos(d: VDouble) -> VDouble {
  VDouble u, s, r = d;
  VInt ql;
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX2).as_vd())))) {
    ...
    VDouble dqh = vtruncate_vd_vd(d.mla((M_1_PI / D1_23).as_vd(), (-M_1_PI / D1_24).as_vd()));
    ql = vrint_vi_vd(vadd_vd_vd_vd(vmul_vd_vd_vd(d, M_1_PI.as_vd()),
    ...
    d = vadd_vd_vd_vd(dqh, dql).mla((-PI_D * 0.5).as_vd(), d);
  } else {
    let (ddidd, ddii) = rempi(d);
    ql = vand_vi_vi_vi(ddii, (3).as_vi());
    ql = ql + ql + vsel_vi_vo_vi_vi(vcast_vo32_vo64(vgt_vo_vd_vd(ddidd.x, (0.).as_vd())), (8).as_vi(), (7).as_vi());
    ql = vsra_vi_vi_i(ql, 1);
    VOpMask o = veq_vo_vi_vi(vand_vi_vi_vi(ddii, (1).as_vi()), (0).as_vi());
    VDouble y = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(ddidd.x, (0.).as_vd()), (0.).as_vd(), (-1.).as_vd());
...
pub fn xcos_u1(d: VDouble) -> VDouble {
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX2).as_vd())))) {
    VDouble dql = vrint_vd_vd(d.mla(M_1_PI.as_vd(), (-0.5).as_vd()));
    dql = (2.).as_vd().mla(dql, (1.).as_vd());
    ql = vrint_vi_vd(dql);
    s = ddadd2_vd2_vd_vd(d, vmul_vd_vd_vd(dql, (-PI_A2*0.5).as_vd()));
    s = ddadd_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_B2*0.5).as_vd()));
  } else if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX.as_vd())))) {
    VDouble dqh = vtruncate_vd_vd(d.mla((M_1_PI / D1_23).as_vd(), (-M_1_PI / D1_24).as_vd()));
    ql = vrint_vi_vd(vadd_vd_vd_vd(vmul_vd_vd_vd(d, M_1_PI.as_vd()),
					dqh.mla((-D1_23).as_vd(), (-0.5).as_vd())));
    ...
    s = ddadd2_vd2_vd_vd(u, vmul_vd_vd_vd(dql, (-PI_A*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dqh, (-PI_B*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_B*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dqh, (-PI_C*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_C*0.5).as_vd()));
    s = ddadd_vd2_vd2_vd(s, vmul_vd_vd_vd(vadd_vd_vd_vd(dqh, dql), (-PI_D*0.5).as_vd()));
  } else {
    let (ddidd, ddii) = rempi(d);
    ql = vand_vi_vi_vi(ddii, (3).as_vi());
    ql = ql + ql + vsel_vi_vo_vi_vi(vcast_vo32_vo64(vgt_vo_vd_vd(ddidd.x, (0.).as_vd())), (8).as_vi(), (7).as_vi());
...
pub fn xsincos(d: VDouble) -> (VDouble, VDouble) {
  VOpMask o;
  VDouble u, t, rx, ry, s;
  VDouble2 r;
  VInt ql;
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX2.as_vd())))) {
    VDouble dql = vrint_vd_vd(vmul_vd_vd_vd(d, (2 * M_1_PI).as_vd()));
    ql = vrint_vi_vd(dql);
    s = dql.mla((-PI_A2 * 0.5).as_vd(), d);
    s = dql.mla((-PI_B2 * 0.5).as_vd(), s);
  } else if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX.as_vd())))) {
    VDouble dqh = vtruncate_vd_vd(vmul_vd_vd_vd(d, (2*M_1_PI / D1_24).as_vd()));
    dqh = vmul_vd_vd_vd(dqh, D1_24.as_vd());
    VDouble dql = vrint_vd_vd(vsub_vd_vd_vd(vmul_vd_vd_vd(d, (2*M_1_PI).as_vd()), dqh));
    ql = vrint_vi_vd(dql);
    ...
    s = vadd_vd_vd_vd(dqh, dql).mla((-PI_D * 0.5).as_vd(), s);
  } else {
    let (ddidd, ddii) = rempi(d);
    ql = ddii;
    s = vadd_vd_vd_vd(ddidd.x, ddidd.y);
...
pub fn xsincos_u1(d: VDouble) -> (VDouble, VDouble) {
  VOpMask o;
  VDouble u, rx, ry;
  VDouble2 r, s, t, x;
  VInt ql;
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX2.as_vd())))) {
    const VDouble dql = vrint_vd_vd(vmul_vd_vd_vd(d, (2 * M_1_PI).as_vd()));
    ql = vrint_vi_vd(dql);
    u = dql.mla((-PI_A2*0.5).as_vd(), d);
    s = ddadd_vd2_vd_vd (u, vmul_vd_vd_vd(dql, (-PI_B2*0.5).as_vd()));
  } else if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX.as_vd())))) {
    VDouble dqh = vtruncate_vd_vd(vmul_vd_vd_vd(d, (2*M_1_PI / D1_24).as_vd()));
    dqh = vmul_vd_vd_vd(dqh, D1_24.as_vd());
    const VDouble dql = vrint_vd_vd(vsub_vd_vd_vd(vmul_vd_vd_vd(d, (2*M_1_PI).as_vd()), dqh));
    ql = vrint_vi_vd(dql);
    
    u = dqh.mla((-PI_A * 0.5).as_vd(), d);
    s = ddadd_vd2_vd_vd(u, vmul_vd_vd_vd(dql, (-PI_A*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dqh, (-PI_B*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_B*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dqh, (-PI_C*0.5).as_vd()));
    s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dql, (-PI_C*0.5).as_vd()));
  ...
  u = (1.58938307283228937328511e-10).as_vd()
      .mla(s.x, (-2.50506943502539773349318e-08).as_vd())
      .mla(s.x, (2.75573131776846360512547e-06).as_vd())
      .mla(s.x, (-0.000198412698278911770864914).as_vd())
      .mla(s.x, (0.0083333333333191845961746).as_vd())
      .mla(s.x, (-0.166666666666666130709393).as_vd());
  ...
  u = (-1.13615350239097429531523e-11).as_vd()
      .mla(s.x, (2.08757471207040055479366e-09).as_vd())
      .mla(s.x, (-2.75573144028847567498567e-07).as_vd())
      .mla(s.x, (2.48015872890001867311915e-05).as_vd())
      .mla(s.x, (-0.00138888888888714019282329).as_vd())
      .mla(s.x, (0.0416666666666665519592062).as_vd())
      .mla(s.x, (-0.5).as_vd());

  x = ddadd_vd2_vd_vd2((1.).as_vd(), ddmul_vd2_vd_vd(s.x, u));
...
pub fn xsincospi_u05(d: VDouble) -> (VDouble, VDouble) {
  VOpMask o;
  VDouble u, s, t, rx, ry;
  VDouble2 r, x, s2;

  u = vmul_vd_vd_vd(d, (4.).as_vd());
  ...
  o = vgt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX3/4.).as_vd());
...
pub fn xsincospi_u35(d: VDouble) -> (VDouble, VDouble) {
  VOpMask o;
  VDouble u, s, t, rx, ry;
  VDouble2 r;
...
pub fn xmodf(x: VDouble) -> (VDouble, VDouble) {
  VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_31.as_vd(), vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd())))));
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  fr = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(vabs_vd_vd(x), D1_52.as_vd()), (0.).as_vd(), fr);

  VDouble2 ret;

  retx = vcopysign_vd_vd_vd(fr, x);
  rety = vcopysign_vd_vd_vd(vsub_vd_vd_vd(x, fr), x);

  (retx, rety)
}

#[inline]
fn sinpik(d: VDouble) -> VDouble2 {
  VOpMask o;
  VDouble u, s, t;
  VDouble2 x, s2;
  ...
  o = vcast_vo64_vo32(veq_vo_vi_vi(vand_vi_vi_vi(q, (4).as_vi()), (4).as_vi()));
  x.x = vreinterpret_vd_vm(vxor_vm_vm_vm(vand_vm_vo64_vm(o, vreinterpret_vm_vd((-0.).as_vd())), vreinterpret_vm_vd(x.x)));
  x.y = vreinterpret_vd_vm(vxor_vm_vm_vm(vand_vm_vo64_vm(o, vreinterpret_vm_vd((-0.).as_vd())), vreinterpret_vm_vd(x.y)));

  return x;
}

pub fn xsinpi_u05(d: VDouble) -> VDouble {
  VDouble2 x = sinpik(d);
  VDouble r = vadd_vd_vd_vd(x.x, x.y);

  r = vsel_vd_vo_vd_vd(visnegzero_vo_vd(d), (-0.).as_vd(), r);
  r = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vgt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX3/4.).as_vd()), vreinterpret_vm_vd(r)));
  r = vreinterpret_vd_vm(vor_vm_vo64_vm(visinf_vo_vd(d), vreinterpret_vm_vd(r)));
  
  return r;
}
#[inline]
fn cospik(d: VDouble) -> VDouble2 {
  VOpMask o;
  VDouble u, s, t;
  VDouble2 x, s2;
...
pub fn xcospi_u05(d: VDouble) -> VDouble {
  VDouble2 x = cospik(d);
  VDouble r = vadd_vd_vd_vd(x.x, x.y);

  r = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(vabs_vd_vd(d), (TRIGRANGEMAX3/4.).as_vd()), (1.).as_vd(), r);
  r = vreinterpret_vd_vm(vor_vm_vo64_vm(visinf_vo_vd(d), vreinterpret_vm_vd(r)));
  
  return r;
}

pub fn xtan(d: VDouble) -> VDouble {
  VDouble u, s, x;
  VOpMask o;
  VInt ql;
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX2.as_vd())))) {
  ...
  } else if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), (1e+7).as_vd())))) {
    VDouble dqh = vtruncate_vd_vd(vmul_vd_vd_vd(d, (2*M_1_PI / D1_24).as_vd()));
    dqh = vmul_vd_vd_vd(dqh, D1_24.as_vd());
    VDouble dql = vrint_vd_vd(vsub_vd_vd_vd(vmul_vd_vd_vd(d, (2*M_1_PI).as_vd()), dqh));
    ql = vrint_vi_vd(dql);
    ...
    x = vadd_vd_vd_vd(dqh, dql).mla((-PI_D * 0.5).as_vd(), x);
  } else {
    let (ddidd, ddii) = rempi(d);
    ql = ddii;
    x = vadd_vd_vd_vd(ddidd.x, ddidd.y);
...
pub fn xtan_u1(d: VDouble) -> VDouble {
  ...
  if (LIKELY(vtestallones_i_vo64(vlt_vo_vd_vd(vabs_vd_vd(d), TRIGRANGEMAX2.as_vd())))) {
    ...
    s = ddadd2_vd2_vd2_vd(ddmul_vd2_vd2_vd(vcast_vd2_d_d(M_2_PI_H, M_2_PI_L), d),
			  vsub_vd_vd_vd(vsel_vd_vo_vd_vd(vlt_vo_vd_vd(d, (0.).as_vd()),
  ...
  u = (1.01419718511083373224408e-05).as_vd()
      .mla(s.x, (-2.59519791585924697698614e-05).as_vd())
      .mla(s.x, (5.23388081915899855325186e-05).as_vd())
      .mla(s.x, (-3.05033014433946488225616e-05).as_vd())
      .mla(s.x, (7.14707504084242744267497e-05).as_vd())
      .mla(s.x, (8.09674518280159187045078e-05).as_vd())
      .mla(s.x, (0.000244884931879331847054404).as_vd())
      .mla(s.x, (0.000588505168743587154904506).as_vd())
      .mla(s.x, (0.00145612788922812427978848).as_vd())
      .mla(s.x, (0.00359208743836906619142924).as_vd())
      .mla(s.x, (0.00886323944362401618113356).as_vd())
      .mla(s.x, (0.0218694882853846389592078).as_vd())
      .mla(s.x, (0.0539682539781298417636002).as_vd())
      .mla(s.x, (0.133333333333125941821962).as_vd());
#endif
  
  x = ddadd_vd2_vd_vd2((1.).as_vd(), ddmul_vd2_vd2_vd2(ddadd_vd2_vd_vd((0.333333333333334980164153).as_vd(), vmul_vd_vd_vd(u, s.x)), s));
...
#[inline]
fn atan2k(y: VDouble, x: VDouble) -> VDouble {
  VDouble s, t, u;
  VInt q;
  VOpMask p;
...
fn atan2k_u1(VDouble2 y, VDouble2 x) -> VDouble2 {
  ...
  q = vsel_vi_vd_vd_vi_vi(x.x, y.x, q + (1).as_vi(), q);
  ...
  u = (0.00070557664296393412389774).as_vd()
      .mla(t.x, (-0.00251865614498713360352999).as_vd())
      .mla(tx3, (0.0208024799924145797902497).as_vd())
      .mla(t.x, (-0.0289002344784740315686289).as_vd())
      .mla(tx3, (0.0470843011653283988193763).as_vd())
      .mla(t.x, (-0.0524914210588448421068719).as_vd())
      .mla(tx3, (0.0769225330296203768654095).as_vd())
      .mla(t.x, (-0.0909090442773387574781907).as_vd())
      .mla(tx3, (0.199999999997977351284817).as_vd())
      .mla(t.x, (-0.333333333333317605173818).as_vd());
  
  v = (1.06298484191448746607415e-05).as_vd()
      .mla(t.x, (-0.000125620649967286867384336).as_vd())
      .mla(tx3, (0.00646262899036991172313504).as_vd())
      .mla(t.x, (-0.0128281333663399031014274).as_vd())
      .mla(tx3, (0.0359785005035104590853656).as_vd())
      .mla(t.x, (-0.041848579703592507506027).as_vd())
      .mla(tx3, (0.0587946590969581003860434).as_vd())
      .mla(t.x, (-0.0666620884778795497194182).as_vd())
      .mla(tx3, (0.111111108376896236538123).as_vd())
      .mla(t.x, (-0.142857142756268568062339).as_vd());
  
  u = v.mla(vmul_vd_vd_vd(t.x, t.x), u);
#else
  u = (1.06298484191448746607415e-05).as_vd()
      .mla(t.x, (-0.000125620649967286867384336).as_vd())
      .mla(t.x, (0.00070557664296393412389774).as_vd())
      .mla(t.x, (-0.00251865614498713360352999).as_vd())
      .mla(t.x, (0.00646262899036991172313504).as_vd())
      .mla(t.x, (-0.0128281333663399031014274).as_vd())
      .mla(t.x, (0.0208024799924145797902497).as_vd())
      .mla(t.x, (-0.0289002344784740315686289).as_vd())
      .mla(t.x, (0.0359785005035104590853656).as_vd())
      .mla(t.x, (-0.041848579703592507506027).as_vd())
      .mla(t.x, (0.0470843011653283988193763).as_vd())
      .mla(t.x, (-0.0524914210588448421068719).as_vd())
      .mla(t.x, (0.0587946590969581003860434).as_vd())
      .mla(t.x, (-0.0666620884778795497194182).as_vd())
      .mla(t.x, (0.0769225330296203768654095).as_vd())
      .mla(t.x, (-0.0909090442773387574781907).as_vd())
      .mla(t.x, (0.111111108376896236538123).as_vd())
      .mla(t.x, (-0.142857142756268568062339).as_vd())
      .mla(t.x, (0.199999999997977351284817).as_vd())
      .mla(t.x, (-0.333333333333317605173818).as_vd());
#endif
...
pub fn xatan2(y: VDouble, x: VDouble) -> VDouble {
  VDouble r = atan2k(vabs_vd_vd(y), x);

  r = vmulsign_vd_vd_vd(r, x);
  r = vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(x), veq_vo_vd_vd(x, (0.).as_vd())), vsub_vd_vd_vd((M_PI/2.).as_vd(), visinf2_vd_vd_vd(x, vmulsign_vd_vd_vd((M_PI/2.).as_vd(), x))), r);
  r = vsel_vd_vo_vd_vd(visinf_vo_vd(y), vsub_vd_vd_vd((M_PI/2.).as_vd(), visinf2_vd_vd_vd(x, vmulsign_vd_vd_vd((M_PI/4.).as_vd(), x))), r);
  r = vsel_vd_vo_vd_vd(veq_vo_vd_vd(y, (0.).as_vd()), vreinterpret_vd_vm(vand_vm_vo64_vm(vsignbit_vo_vd(x), vreinterpret_vm_vd(M_PI.as_vd()))), r);

  r = vreinterpret_vd_vm(vor_vm_vo64_vm(vor_vo_vo_vo(visnan_vo_vd(x), visnan_vo_vd(y)), vreinterpret_vm_vd(vmulsign_vd_vd_vd(r, y))));
  return r;
}

pub fn xatan2_u1(y: VDouble, x: VDouble) -> VDouble {
  VOpMask o = vlt_vo_vd_vd(vabs_vd_vd(x), (5.5626846462680083984e-309).as_vd()); // nexttoward((1.0 / DBL_MAX), 1)
  x = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(x, D1_53.as_vd(), x);
  y = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(y, D1_23.as_vd()), y);
...
pub fn xasin(d: VDouble) -> VDouble {
  VOpMask o = vlt_vo_vd_vd(vabs_vd_vd(d), (0.5).as_vd());
  VDouble x2 = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, d), vmul_vd_vd_vd(vsub_vd_vd_vd((1.).as_vd(), vabs_vd_vd(d)), (0.5).as_vd()));
  ...
  u = u.mla(vmul_vd_vd_vd(x, x2), x);
  
  VDouble r = vsel_vd_vo_vd_vd(o, u, u.mla((-2.).as_vd(), (M_PI/2.).as_vd()));
  return vmulsign_vd_vd_vd(r, d);
}

pub fn xasin_u1(d: VDouble) -> VDouble {
  VOpMask o = vlt_vo_vd_vd(vabs_vd_vd(d), (0.5).as_vd());
  VDouble x2 = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, d), vmul_vd_vd_vd(vsub_vd_vd_vd((1.).as_vd(), vabs_vd_vd(d)), (0.5).as_vd())), u;
  VDouble2 x = vsel_vd2_vo_vd2_vd2(o, vcast_vd2_vd_vd(vabs_vd_vd(d), (0.).as_vd()), ddsqrt_vd2_vd(x2));
  x = vsel_vd2_vo_vd2_vd2(veq_vo_vd_vd(vabs_vd_vd(d), (1.).as_vd()), vcast_vd2_d_d(0, 0), x);
  ...
  VDouble r = vsel_vd_vo_vd_vd(o, vadd_vd_vd_vd(u, x.x),
			       vmul_vd_vd_vd(vadd_vd_vd_vd(y.x, y.y), (2.).as_vd()));
  return vmulsign_vd_vd_vd(r, d);
}

pub fn xacos(d: VDouble) -> VDouble {
  VOpMask o = vlt_vo_vd_vd(vabs_vd_vd(d), (0.5).as_vd());
  VDouble x2 = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, d),
				vmul_vd_vd_vd(vsub_vd_vd_vd((1.).as_vd(), vabs_vd_vd(d)), (0.5).as_vd())), u;
  VDouble x = vsel_vd_vo_vd_vd(o, vabs_vd_vd(d), vsqrt_vd_vd(x2));
  x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(vabs_vd_vd(d), (1.).as_vd()), (0.).as_vd(), x);
  ...
  VDouble y = vsub_vd_vd_vd((M_PI/2.).as_vd(), vadd_vd_vd_vd(vmulsign_vd_vd_vd(x, d), vmulsign_vd_vd_vd(u, d)));
  x = vadd_vd_vd_vd(x, u);
  VDouble r = vsel_vd_vo_vd_vd(o, y, vmul_vd_vd_vd(x, (2.).as_vd()));
  return vsel_vd_vo_vd_vd(vandnot_vo_vo_vo(o, vlt_vo_vd_vd(d, (0.).as_vd())),
...
pub fn xacos_u1(d: VDouble) -> VDouble {
  ...
  y = vsel_vd2_vo_vd2_vd2(vandnot_vo_vo_vo(o, vlt_vo_vd_vd(d, (0.).as_vd())),
...
pub fn xatan(s: VDouble) -> VDouble {
  VDouble t, u;
  VInt q;
  ...
  s = vsel_vd_vo_vd_vd(vlt_vo_vd_vd((1.).as_vd(), s), vrec_vd_vd(s), s);
  ...
  t = vsel_vd_vo_vd_vd(vcast_vo64_vo32(veq_vo_vi_vi(vand_vi_vi_vi(q, (1).as_vi()), (1).as_vi())), vsub_vd_vd_vd((M_PI/2.).as_vd(), t), t);
...
#if defined(__INTEL_COMPILER) && defined(ENABLE_PURECFMA_SCALAR)
  t = vsel_vd_vo_vd_vd(veq_vo_vd_vd(w, (0.).as_vd()), w, t);
#endif

  return t;
}

pub fn xlog(d: VDouble) -> VDouble {
  VDouble x, x2;
  VDouble t, m;
  
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  VOpMask o = vlt_vo_vd_vd(d, f64::MIN.as_vd());
  d = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, (D1_32 * D1_32).as_vd()), d);
  VInt e = vilogb2k_vi_vd(vmul_vd_vd_vd(d, (1./0.75).as_vd()));
  m = vldexp3_vd_vd_vi(d, vneg_vi_vi(e));
  e = vsel_vi_vo_vi_vi(vcast_vo32_vo64(o), e - (64).as_vi(), e);
#else
  VDouble e = vgetexp_vd_vd(vmul_vd_vd_vd(d, (1./0.75).as_vd()));
  e = vsel_vd_vo_vd_vd(vispinf_vo_vd(e), (1024.).as_vd(), e);
  m = vgetmant_vd_vd(d);
#endif
  
  x = vdiv_vd_vd_vd(vadd_vd_vd_vd((-1.).as_vd(), m), vadd_vd_vd_vd((1.).as_vd(), m));
...
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  x = x.mla(t, vmul_vd_vd_vd((0.693147180559945286226764).as_vd(), vcast_vd_vi(e)));

  x = vsel_vd_vo_vd_vd(vispinf_vo_vd(d), SLEEF_INFINITY.as_vd(), x);
  x = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vlt_vo_vd_vd(d, (0.).as_vd()), visnan_vo_vd(d)), (SLEEF_NAN).as_vd(), x);
  x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.).as_vd()), (-SLEEF_INFINITY).as_vd(), x);
#else
  x = x.mla(t, vmul_vd_vd_vd((0.693147180559945286226764).as_vd(), e));
...
pub fn xexp(d: VDouble) -> VDouble {
  VDouble u = vrint_vd_vd(vmul_vd_vd_vd(d, R_LN2.as_vd())), s;
  ...
  u = vadd_vd_vd_vd((1.).as_vd(), vmul_vd_vd_vd(s, s).mla(u, s));
#endif // #ifdef ENABLE_FMA_DP
  
  u = vldexp2_vd_vd_vi(u, q);

  u = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(d, (709.78271114955742909217217426).as_vd()), SLEEF_INFINITY.as_vd(), u);
  u = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(d, (-1000.).as_vd()), vreinterpret_vm_vd(u)));

  return u;
}
#[inline]
fn expm1k(d: VDouble) -> VDouble {
  VDouble u = vrint_vd_vd(vmul_vd_vd_vd(d, R_LN2.as_vd())), s;
  VInt q = vrint_vi_vd(u);
  ...
  u = (2.08860621107283687536341e-09).as_vd()
      ...
      .mla(s, (0.166666666666666851703837).as_vd())
      .mla(s, (0.5).as_vd());
  u = vmul_vd_vd_vd(s, s).mla(u, s);
  
  u = vsel_vd_vo_vd_vd(vcast_vo64_vo32(veq_vo_vi_vi(q, (0).as_vi())), u,
		       vsub_vd_vd_vd(vldexp2_vd_vd_vi(vadd_vd_vd_vd(u, (1.).as_vd()), q), (1.).as_vd()));

  return u;
}
#[inline]
fn logk(d: VDouble) -> VDouble2 {
  VDouble2 x, x2, s;
  VDouble t, m;
  ...
  t = (0.116255524079935043668677).as_vd()
      .mla(x2.x, (0.103239680901072952701192).as_vd())
      .mla(x2.x, (0.117754809412463995466069).as_vd())
      .mla(x2.x, (0.13332981086846273921509).as_vd())
      .mla(x2.x, (0.153846227114512262845736).as_vd())
      .mla(x2.x, (0.181818180850050775676507).as_vd())
      .mla(x2.x, (0.222222222230083560345903).as_vd())
      .mla(x2.x, (0.285714285714249172087875).as_vd())
      .mla(x2.x, (0.400000000000000077715612).as_vd());
...
pub fn xlog_u1(d: VDouble) -> VDouble {
  VDouble2 x;
  VDouble t, m, x2;
  ...
  r = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vlt_vo_vd_vd(d, (0.).as_vd()), visnan_vo_vd(d)), (SLEEF_NAN).as_vd(), r);
  r = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.).as_vd()), (-SLEEF_INFINITY).as_vd(), r);
#else
...
#[inline]
fn expk(VDouble2 d) -> VDouble {
  VDouble u = vmul_vd_vd_vd(vadd_vd_vd_vd(d.x, d.y), R_LN2.as_vd());
  VDouble dq = vrint_vd_vd(u);
  VInt q = vrint_vi_vd(dq);
  VDouble2 s, t;

  s = ddadd2_vd2_vd2_vd(d, vmul_vd_vd_vd(dq, (-L2U).as_vd()));
  s = ddadd2_vd2_vd2_vd(s, vmul_vd_vd_vd(dq, (-L2L).as_vd()));

  s = ddnormalize_vd2_vd2(s);

  u = (2.51069683420950419527139e-08).as_vd()
      .mla(s.x, (2.76286166770270649116855e-07).as_vd())
      .mla(s.x, (2.75572496725023574143864e-06).as_vd())
      .mla(s.x, (2.48014973989819794114153e-05).as_vd())
      .mla(s.x, (0.000198412698809069797676111).as_vd())
      .mla(s.x, (0.0013888888939977128960529).as_vd())
      .mla(s.x, (0.00833333333332371417601081).as_vd())
      .mla(s.x, (0.0416666666665409524128449).as_vd())
      .mla(s.x, (0.166666666666666740681535).as_vd())
      .mla(s.x, (0.500000000000000999200722).as_vd());
  ...
  u = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(d.x, (-1000.).as_vd()), vreinterpret_vm_vd(u)));
...
pub fn xpow(x: VDouble, y: VDouble) -> VDouble {
  ...
  result = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(d.x, (709.78271114955742909217217426).as_vd()), SLEEF_INFINITY.as_vd(), result);

  result = vmul_vd_vd_vd(result,
			 vsel_vd_vo_vd_vd(vgt_vo_vd_vd(x, (0.).as_vd()),
					  (1.).as_vd(),
					  vsel_vd_vo_vd_vd(yisint, vsel_vd_vo_vd_vd(yisodd, (-1.).as_vd(), (1.).as_vd()), (SLEEF_NAN).as_vd())));

  VDouble efx = vmulsign_vd_vd_vd(vsub_vd_vd_vd(vabs_vd_vd(x), (1.).as_vd()), y);

  result = vsel_vd_vo_vd_vd(visinf_vo_vd(y),
			    vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(efx, (0.).as_vd()),
								  vreinterpret_vm_vd(vsel_vd_vo_vd_vd(veq_vo_vd_vd(efx, (0.).as_vd()),
												      (1.).as_vd(),
												      SLEEF_INFINITY.as_vd())))),
			    result);

  result = vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(x), veq_vo_vd_vd(x, (0.).as_vd())),
			    vmul_vd_vd_vd(vsel_vd_vo_vd_vd(yisodd, vsign_vd_vd(x), (1.).as_vd()),
					  vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(vsel_vd_vo_vd_vd(veq_vo_vd_vd(x, (0.).as_vd()), vneg_vd_vd(y), y), (0.).as_vd()),
  ...
  result = vsel_vd_vo_vd_vd(vor_vo_vo_vo(veq_vo_vd_vd(y, (0.).as_vd()), veq_vo_vd_vd(x, (1.).as_vd())), (1.).as_vd(), result);
...
fn expk2(VDouble2 d) -> VDouble2 {
  ...
  u = (+0.1602472219709932072e-9).as_vd()
      .mla(s.x, (+0.2092255183563157007e-8).as_vd())
      .mla(s.x, (+0.2505230023782644465e-7).as_vd())
      .mla(s.x, (+0.2755724800902135303e-6).as_vd())
      .mla(s.x, (+0.2755731892386044373e-5).as_vd())
      .mla(s.x, (+0.2480158735605815065e-4).as_vd())
      .mla(s.x, (+0.1984126984148071858e-3).as_vd())
      .mla(s.x, (+0.1388888888886763255e-2).as_vd())
      .mla(s.x, (+0.8333333333333347095e-2).as_vd())
      .mla(s.x, (+0.4166666666666669905e-1).as_vd());
  ...
  t.x = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(d.x, (-1000.).as_vd()), vreinterpret_vm_vd(t.x)));
  t.y = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(d.x, (-1000.).as_vd()), vreinterpret_vm_vd(t.y)));
...
pub fn xsinh(x: VDouble) -> VDouble {
  ...
  y = vmul_vd_vd_vd(vadd_vd_vd_vd(d.x, d.y), (0.5).as_vd());

  y = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vgt_vo_vd_vd(vabs_vd_vd(x), (710.).as_vd()), visnan_vo_vd(y)), SLEEF_INFINITY.as_vd(), y);
...
pub fn xtanh(x: VDouble) -> VDouble {
  ...
  y = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vgt_vo_vd_vd(vabs_vd_vd(x), (18.714973875).as_vd()), visnan_vo_vd(y)), (1.).as_vd(), y);
  y = vmulsign_vd_vd_vd(y, x);
  y = vreinterpret_vd_vm(vor_vm_vo64_vm(visnan_vo_vd(x), vreinterpret_vm_vd(y)));
...
pub fn xsinh_u35(x: VDouble) -> VDouble {
  VDouble e = expm1k(vabs_vd_vd(x));

  VDouble y = vdiv_vd_vd_vd(vadd_vd_vd_vd(e, (2.).as_vd()), vadd_vd_vd_vd(e, (1.).as_vd()));
  y = vmul_vd_vd_vd(y, vmul_vd_vd_vd((0.5).as_vd(), e));

  y = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vgt_vo_vd_vd(vabs_vd_vd(x), (709.).as_vd()), visnan_vo_vd(y)), SLEEF_INFINITY.as_vd(), y);
  y = vmulsign_vd_vd_vd(y, x);
  y = vreinterpret_vd_vm(vor_vm_vo64_vm(visnan_vo_vd(x), vreinterpret_vm_vd(y)));
...
pub fn xcosh_u35(x: VDouble) -> VDouble {
  VDouble e = xexp(vabs_vd_vd(x));
  VDouble y = (0.5).as_vd().mla(e, vdiv_vd_vd_vd((0.5).as_vd(), e));
...
pub fn xtanh_u35(x: VDouble) -> VDouble {
  VDouble d = expm1k(vmul_vd_vd_vd((2.).as_vd(), vabs_vd_vd(x)));
  VDouble y = vdiv_vd_vd_vd(d, vadd_vd_vd_vd((2.).as_vd(), d));
...
fn logk2(VDouble2 d) -> VDouble2 {
  ...
  e = vilogbk_vi_vd(vmul_vd_vd_vd(d.x, (1./0.75).as_vd()));
  ...
  t = (0.13860436390467167910856).as_vd()
      .mla(x2.x, (0.131699838841615374240845).as_vd())
      .mla(x2.x, (0.153914168346271945653214).as_vd())
      .mla(x2.x, (0.181816523941564611721589).as_vd())
      .mla(x2.x, (0.22222224632662035403996).as_vd())
      .mla(x2.x, (0.285714285511134091777308).as_vd())
      .mla(x2.x, (0.400000000000914013309483).as_vd())
      .mla(x2.x, (0.666666666666664853302393).as_vd());
...
pub fn xasinh(x: VDouble) -> VDouble {
  VDouble y = vabs_vd_vd(x);
  VOpMask o = vgt_vo_vd_vd(y, (1.).as_vd());
  VDouble2 d;
  ...
  y = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vgt_vo_vd_vd(vabs_vd_vd(x), SQRT_DBL_MAX.as_vd()),
				    visnan_vo_vd(y)),
...
pub fn xacosh(x: VDouble) -> VDouble {
  ...
  y = vreinterpret_vd_vm(vandnot_vm_vo64_vm(veq_vo_vd_vd(x, (1.).as_vd()), vreinterpret_vm_vd(y)));

  y = vreinterpret_vd_vm(vor_vm_vo64_vm(vlt_vo_vd_vd(x, (1.).as_vd()), vreinterpret_vm_vd(y)));
  y = vreinterpret_vd_vm(vor_vm_vo64_vm(visnan_vo_vd(x), vreinterpret_vm_vd(y)));
...
pub fn xatanh(x: VDouble) -> VDouble {
  VDouble y = vabs_vd_vd(x);
  VDouble2 d = logk2(dddiv_vd2_vd2_vd2(ddadd2_vd2_vd_vd((1.).as_vd(), y), ddadd2_vd2_vd_vd((1.).as_vd(), vneg_vd_vd(y))));
  y = vreinterpret_vd_vm(vor_vm_vo64_vm(vgt_vo_vd_vd(y, (1.).as_vd()), vreinterpret_vm_vd(vsel_vd_vo_vd_vd(veq_vo_vd_vd(y, (1.).as_vd()), SLEEF_INFINITY.as_vd(), vmul_vd_vd_vd(vadd_vd_vd_vd(d.x, d.y), (0.5).as_vd())))));
...
pub fn xcbrt(d: VDouble) -> VDouble {
  VDouble x, y, q = (1.).as_vd();
  VInt e, qu, re;
  VDouble t;
  ...
  t = vadd_vd_vd_vd(vcast_vd_vi(e), (6144.).as_vd());
  qu = vtruncate_vi_vd(vmul_vd_vd_vd(t, (1./3.).as_vd()));
  re = vtruncate_vi_vd(vsub_vd_vd_vd(t, vmul_vd_vd_vd(vcast_vd_vi(qu), (3.).as_vd())));
  ...
  y = vmul_vd_vd_vd(x, x); y = vmul_vd_vd_vd(y, y); x = vsub_vd_vd_vd(x, vmul_vd_vd_vd(vmlapn_vd_vd_vd_vd(d, y, x), (1. / 3.).as_vd()));
  y = vmul_vd_vd_vd(vmul_vd_vd_vd(d, x), x);
  y = vmul_vd_vd_vd(vsub_vd_vd_vd(y, vmul_vd_vd_vd(vmul_vd_vd_vd((2. / 3.).as_vd(), y), y.mla(x, (-1.).as_vd()))), q);

#if defined(ENABLE_AVX512F) || defined(ENABLE_AVX512FNOFMA)
  y = vsel_vd_vo_vd_vd(visinf_vo_vd(s), vmulsign_vd_vd_vd(SLEEF_INFINITY.as_vd(), s), y);
  y = vsel_vd_vo_vd_vd(veq_vo_vd_vd(s, (0.).as_vd()), vmulsign_vd_vd_vd((0.).as_vd(), s), y);
#endif
...
pub fn xcbrt_u1(d: VDouble) -> VDouble {
  ...
  y = vmul_vd_vd_vd(vmul_vd_vd_vd((-2. / 3.).as_vd(), y), z);
  v = ddadd2_vd2_vd2_vd(ddmul_vd2_vd_vd(z, z), y);
  v = ddmul_vd2_vd2_vd(v, d);
  v = ddmul_vd2_vd2_vd2(v, q2);
  z = vldexp2_vd_vd_vi(vadd_vd_vd_vd(v.x, v.y), qu - (2048).as_vi());

#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  z = vsel_vd_vo_vd_vd(visinf_vo_vd(d), vmulsign_vd_vd_vd(SLEEF_INFINITY.as_vd(), q2.x), z);
  z = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.).as_vd()), vreinterpret_vd_vm(vsignbit_vm_vd(q2.x)), z);
#else
  z = vsel_vd_vo_vd_vd(visinf_vo_vd(s), vmulsign_vd_vd_vd(SLEEF_INFINITY.as_vd(), s), z);
  z = vsel_vd_vo_vd_vd(veq_vo_vd_vd(s, (0.).as_vd()), vmulsign_vd_vd_vd((0.).as_vd(), s), z);
#endif
...
#else
  u = ddnormalize_vd2_vd2(ddadd_vd2_vd_vd2((1.).as_vd(), ddmul_vd2_vd_vd(u, s))).x;
#endif
  
  u = vldexp2_vd_vd_vi(u, q);

  u = vsel_vd_vo_vd_vd(vge_vo_vd_vd(d, (1024.).as_vd()), SLEEF_INFINITY.as_vd(), u);
  u = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(d, (-2000.).as_vd()), vreinterpret_vm_vd(u)));

  return u;
}

pub fn xexp10(d: VDouble) -> VDouble {
  VDouble u = vrint_vd_vd(vmul_vd_vd_vd(d, LOG10_2.as_vd())), s;
  VInt q = vrint_vi_vd(u);
  ...
  u = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(d, (308.25471555991671).as_vd()), SLEEF_INFINITY.as_vd(), u);
  u = vreinterpret_vd_vm(vandnot_vm_vo64_vm(vlt_vo_vd_vd(d, (-350.).as_vd()), vreinterpret_vm_vd(u)));
...
pub fn xexpm1(a: VDouble) -> VDouble {
  ...
  x = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(a, (709.782712893383996732223).as_vd()), SLEEF_INFINITY.as_vd(), x);
  x = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(a, (-36.736800569677101399113302437).as_vd()), (-1.).as_vd(), x);
  x = vsel_vd_vo_vd_vd(visnegzero_vo_vd(a), (-0.).as_vd(), x);
  return x;
}

pub fn xlog10(d: VDouble) -> VDouble {
  VDouble2 x;
  VDouble t, m, x2;
...
pub fn xlog2(d: VDouble) -> VDouble {
  VDouble2 x;
  VDouble t, m, x2;
  ...
  e = vsel_vi_vo_vi_vi(vcast_vo32_vo64(o), e - (64).as_vi(), e);
#else
  VDouble e = vgetexp_vd_vd(vmul_vd_vd_vd(d, (1.0/0.75).as_vd()));
  e = vsel_vd_vo_vd_vd(vispinf_vo_vd(e), (1024.).as_vd(), e);
  m = vgetmant_vd_vd(d);
#endif
...
pub fn xlog1p(d: VDouble) -> VDouble {
  ...
  VDouble dp1 = vadd_vd_vd_vd(d, (1.).as_vd());

#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  VOpMask o = vlt_vo_vd_vd(dp1, f64::MIN.as_vd());
  dp1 = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(dp1, (D1_32 * D1_32).as_vd()), dp1);
  VInt e = vilogb2k_vi_vd(vmul_vd_vd_vd(dp1, (1./0.75).as_vd()));
  t = vldexp3_vd_vd_vi((1.).as_vd(), vneg_vi_vi(e));
  m = d.mla(t, vsub_vd_vd_vd(t, (1.).as_vd()));
  e = vsel_vi_vo_vi_vi(vcast_vo32_vo64(o), e - (64).as_vi(), e);
  VDouble2 s = ddmul_vd2_vd2_vd(vcast_vd2_d_d(0.693147180559945286226764, 2.319046813846299558417771e-17), vcast_vd_vi(e));
#else
  VDouble e = vgetexp_vd_vd(vmul_vd_vd_vd(dp1, (1./0.75).as_vd()));
  e = vsel_vd_vo_vd_vd(vispinf_vo_vd(e), (1024.).as_vd(), e);
  ...
  r = vsel_vd_vo_vd_vd(vgt_vo_vd_vd(d, (1e+307).as_vd()), SLEEF_INFINITY.as_vd(), r);
  r = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vlt_vo_vd_vd(d, (-1.).as_vd()), visnan_vo_vd(d)), SLEEF_NAN.as_vd(), r);
  r = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (-1.).as_vd()), (-SLEEF_INFINITY).as_vd(), r);
  r = vsel_vd_vo_vd_vd(visnegzero_vo_vd(d), (-0.).as_vd(), r);
...
pub fn xfabs(x: VDouble) -> VDouble { return vabs_vd_vd(x); }

pub fn xcopysign(x: VDouble, y: VDouble) -> VDouble { return vcopysign_vd_vd_vd(x, y); }
...
pub fn xfdim(x: VDouble, y: VDouble) -> VDouble {
  ...
  ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vlt_vo_vd_vd(ret, (0.).as_vd()), veq_vo_vd_vd(x, y)), (0.).as_vd(), ret);
  return ret;
}

pub fn xtrunc(x: VDouble) -> VDouble {
  VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_31.as_vd(), vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd())))));
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  return vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(x), vge_vo_vd_vd(vabs_vd_vd(x), D1_52.as_vd())), x, vcopysign_vd_vd_vd(vsub_vd_vd_vd(x, fr), x));
}

pub fn xfloor(x: VDouble) -> VDouble {
  VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_31.as_vd(), vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd())))));
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  fr = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(fr, (0.).as_vd()), vadd_vd_vd_vd(fr, (1.).as_vd()), fr);
  return vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(x), vge_vo_vd_vd(vabs_vd_vd(x), D1_52.as_vd())), x, vcopysign_vd_vd_vd(vsub_vd_vd_vd(x, fr), x));
}

pub fn xceil(x: VDouble) -> VDouble {
  VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_31.as_vd(), vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd())))));
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  fr = vsel_vd_vo_vd_vd(vle_vo_vd_vd(fr, (0.).as_vd()), fr, vsub_vd_vd_vd(fr, (1.).as_vd()));
  return vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(x), vge_vo_vd_vd(vabs_vd_vd(x), D1_52.as_vd())), x, vcopysign_vd_vd_vd(vsub_vd_vd_vd(x, fr), x));
}

pub fn xround(d: VDouble) -> VDouble {
  VDouble x = vadd_vd_vd_vd(d, (0.5).as_vd());
  VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_31.as_vd(), vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd())))));
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  x = vsel_vd_vo_vd_vd(vand_vo_vo_vo(vle_vo_vd_vd(x, (0.).as_vd()), veq_vo_vd_vd(fr, (0.).as_vd())), vsub_vd_vd_vd(x, (1.).as_vd()), x);
  fr = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(fr, (0.).as_vd()), vadd_vd_vd_vd(fr, (1.).as_vd()), fr);
  x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.49999999999999994449).as_vd()), (0.).as_vd(), x);
  return vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(d), vge_vo_vd_vd(vabs_vd_vd(d), D1_52.as_vd())), d, vcopysign_vd_vd_vd(vsub_vd_vd_vd(x, fr), d));
}

pub fn xrint(d: VDouble) -> VDouble {
  VDouble x = vadd_vd_vd_vd(d, (0.5).as_vd());
  VDouble fr = vsub_vd_vd_vd(x, vmul_vd_vd_vd(D1_31.as_vd(), vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd())))));
  VOpMask isodd = vcast_vo64_vo32(veq_vo_vi_vi(vand_vi_vi_vi((1).as_vi(), vtruncate_vi_vd(fr)), (1).as_vi()));
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  fr = vsel_vd_vo_vd_vd(vor_vo_vo_vo(vlt_vo_vd_vd(fr, (0).as_vd()), vand_vo_vo_vo(veq_vo_vd_vd(fr, (0).as_vd()), isodd)), vadd_vd_vd_vd(fr, (1.).as_vd()), fr);
  x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.50000000000000011102).as_vd()), (0.).as_vd(), x);
  VDouble ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(visinf_vo_vd(d), vge_vo_vd_vd(vabs_vd_vd(d), D1_52.as_vd())), d, vcopysign_vd_vd_vd(vsub_vd_vd_vd(x, fr), d));
  return ret;
}

pub fn xnextafter(x: VDouble, y: VDouble) -> VDouble {
  x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(x, (0.).as_vd()), vmulsign_vd_vd_vd((0.).as_vd(), y), x);
  ...
  ret = vsel_vd_vo_vd_vd(vand_vo_vo_vo(veq_vo_vd_vd(ret, (0.).as_vd()), vneq_vo_vd_vd(x, (0.).as_vd())), 
			 vmulsign_vd_vd_vd((0.).as_vd(), x), ret);

  ret = vsel_vd_vo_vd_vd(vand_vo_vo_vo(veq_vo_vd_vd(x, (0.).as_vd()), veq_vo_vd_vd(y, (0.).as_vd())), y, ret);

  ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(visnan_vo_vd(x), visnan_vo_vd(y)), (SLEEF_NAN).as_vd(), ret);
  
  return ret;
}

pub fn xfrfrexp(x: VDouble) -> VDouble {
  x = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(vabs_vd_vd(x), f64::MIN.as_vd()), vmul_vd_vd_vd(x, D1_63.as_vd()), x);
  ...
  ret = vsel_vd_vo_vd_vd(veq_vo_vd_vd(x, (0.).as_vd()), x, ret);
  
  return ret;
}

pub fn xexpfrexp(x: VDouble) -> VInt {
  x = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(vabs_vd_vd(x), f64::MIN.as_vd()), vmul_vd_vd_vd(x, D1_63.as_vd()), x);

  let ret = vreinterpret_vi2_vd(x).as_vi();
  ret = vand_vi_vi_vi(vsrl_vi_vi_i(ret, 20), vcast_vi_i(0x7ff)) - vcast_vi_i(0x3fe);

  ret = vsel_vi_vo_vi_vi(vor_vo_vo_vo(vor_vo_vo_vo(veq_vo_vd_vd(x, (0.).as_vd()), visnan_vo_vd(x)), visinf_vo_vd(x)), (0).as_vi(), ret);
  
  return ret;
}

pub fn xfma(VDouble x, VDouble y, VDouble z) -> VDouble {
  VDouble h2 = vadd_vd_vd_vd(vmul_vd_vd_vd(x, y), z), q = (1.).as_vd();
  VOpMask o = vlt_vo_vd_vd(vabs_vd_vd(h2), (1e-300).as_vd());
  const C0 : f64 = D1_54;
  const C1 : f64 = C0 * C0;
  const C2 : f64 = C1 * C1;
  {
    x = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(x, C1.as_vd()), x);
    y = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(y, C1.as_vd()), y);
    z = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(z, C2.as_vd()), z);
    q = vsel_vd_vo_vd_vd(o, (1. / C2).as_vd(), q);
  }
  o = vgt_vo_vd_vd(vabs_vd_vd(h2), (1e+300).as_vd());
  {
    x = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(x, (1. / C1).as_vd()), x);
    y = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(y, (1. / C1).as_vd()), y);
    z = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(z, (1. / C2).as_vd()), z);
    q = vsel_vd_vo_vd_vd(o, C2.as_vd(), q);
  }
  VDouble2 d = ddmul_vd2_vd_vd(x, y);
  d = ddadd2_vd2_vd2_vd(d, z);
  VDouble ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(veq_vo_vd_vd(x, (0.).as_vd()), veq_vo_vd_vd(y, (0.).as_vd())), z, vadd_vd_vd_vd(d.x, d.y));
  o = visinf_vo_vd(z);
...
SQRTU05_FUNCATR VDouble xsqrt_u05(d: VDouble) {
  VDouble q;
  VOpMask o;
  
  d = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(d, (0.).as_vd()), (SLEEF_NAN).as_vd(), d);

  o = vlt_vo_vd_vd(d, (8.636168555094445e-78).as_vd());
  d = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, (1.157920892373162e77).as_vd()), d);
  q = vsel_vd_vo_vd_vd(o, (2.9387358770557188e-39*0.5).as_vd(), (0.5).as_vd());

  o = vgt_vo_vd_vd(d, (1.3407807929942597e+154).as_vd());
  d = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, (7.4583407312002070e-155).as_vd()), d);
  q = vsel_vd_vo_vd_vd(o, (1.1579208923731620e+77*0.5).as_vd(), q);

  VDouble x = vreinterpret_vd_vi2(vsub_vi2_vi2_vi2(vcast_vi2_i_i(0x5fe6ec86, 0), vsrl_vi2_vi2_i(vreinterpret_vi2_vd(vadd_vd_vd_vd(d, (1e-320).as_vd())), 1)));

  x = vmul_vd_vd_vd(x, vsub_vd_vd_vd((1.5).as_vd(), vmul_vd_vd_vd(vmul_vd_vd_vd(vmul_vd_vd_vd((0.5).as_vd(), d), x), x)));
  x = vmul_vd_vd_vd(x, vsub_vd_vd_vd((1.5).as_vd(), vmul_vd_vd_vd(vmul_vd_vd_vd(vmul_vd_vd_vd((0.5).as_vd(), d), x), x)));
  x = vmul_vd_vd_vd(x, vsub_vd_vd_vd((1.5).as_vd(), vmul_vd_vd_vd(vmul_vd_vd_vd(vmul_vd_vd_vd((0.5).as_vd(), d), x), x)));
  x = vmul_vd_vd_vd(x, d);
  ...
  x = vsel_vd_vo_vd_vd(vispinf_vo_vd(d), SLEEF_INFINITY.as_vd(), x);
  x = vsel_vd_vo_vd_vd(veq_vo_vd_vd(d, (0.).as_vd()), d, x);
...
pub fn xsqrt_u35(d: VDouble) -> VDouble { return xsqrt_u05(d); }

pub fn xhypot_u05(x: VDouble, y: VDouble) -> VDouble {
  x = vabs_vd_vd(x);
  y = vabs_vd_vd(y);
  VDouble min = vmin_vd_vd_vd(x, y), n = min;
  VDouble max = vmax_vd_vd_vd(x, y), d = max;

  VOpMask o = vlt_vo_vd_vd(max, f64::MIN.as_vd());
  n = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(n, D1_54.as_vd()), n);
  d = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(d, D1_54.as_vd()), d);
  ...
  ret = vsel_vd_vo_vd_vd(veq_vo_vd_vd(min, (0.).as_vd()), max, ret);
  ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(visnan_vo_vd(x), visnan_vo_vd(y)), (SLEEF_NAN).as_vd(), ret);
  ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(veq_vo_vd_vd(x, SLEEF_INFINITY.as_vd()), veq_vo_vd_vd(y, SLEEF_INFINITY.as_vd())), SLEEF_INFINITY.as_vd(), ret);

  return ret;
}

pub fn xhypot_u35(x: VDouble, y: VDouble) -> VDouble {
  x = vabs_vd_vd(x);
  y = vabs_vd_vd(y);
  ...
  VDouble ret = vmul_vd_vd_vd(max, vsqrt_vd_vd(t.mla(t, (1.).as_vd())));
  ret = vsel_vd_vo_vd_vd(veq_vo_vd_vd(min, (0.).as_vd()), max, ret);
  ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(visnan_vo_vd(x), visnan_vo_vd(y)), (SLEEF_NAN).as_vd(), ret);
  ret = vsel_vd_vo_vd_vd(vor_vo_vo_vo(veq_vo_vd_vd(x, SLEEF_INFINITY.as_vd()), veq_vo_vd_vd(y, SLEEF_INFINITY.as_vd())), SLEEF_INFINITY.as_vd(), ret);
...
#[inline]
fn vtoward0(x: VDouble) -> VDouble { // returns nextafter(x, 0)
  VDouble t = vreinterpret_vd_vm(vadd64_vm_vm_vm(vreinterpret_vm_vd(x), vcast_vm_i_i(-1, -1)));
  return vsel_vd_vo_vd_vd(veq_vo_vd_vd(x, (0.).as_vd()), (0.).as_vd(), t);
}
#[inline]
fn vptrunc(x: VDouble) -> VDouble { // round to integer toward 0, positive argument only
#ifdef FULL_FP_ROUNDING
  return vtruncate_vd_vd(x);
#else
  VDouble fr = (-D1_31).as_vd().mla(vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(x, (1. / D1_31).as_vd()))), x);
  fr = vsub_vd_vd_vd(fr, vcast_vd_vi(vtruncate_vi_vd(fr)));
  return vsel_vd_vo_vd_vd(vge_vo_vd_vd(vabs_vd_vd(x), D1_52.as_vd()), x, vsub_vd_vd_vd(x, fr));
#endif
}

/* TODO AArch64: potential optimization by using `vfmad_lane_f64` */
pub fn xfmod(x: VDouble, y: VDouble) -> VDouble {
  VDouble nu = vabs_vd_vd(x), de = vabs_vd_vd(y), s = (1.).as_vd(), q;
  VOpMask o = vlt_vo_vd_vd(de, f64::MIN.as_vd());
  nu = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(nu, D1_54.as_vd()), nu);
  de = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(de, D1_54.as_vd()), de);
  s  = vsel_vd_vo_vd_vd(o, vmul_vd_vd_vd(s , (1. / D1_54).as_vd()), s);
  ...
  for(int i=0;i<21;i++) { // ceil(log2(DBL_MAX) / 51) + 1
    q = vsel_vd_vo_vd_vd(vand_vo_vo_vo(vgt_vo_vd_vd(vadd_vd_vd_vd(de, de), r.x),
				       vge_vo_vd_vd(r.x, de)),
			 (1.).as_vd(), vmul_vd_vd_vd(vtoward0(r.x), rde));
  ...
  ret = vsel_vd_vo_vd_vd(veq_vo_vd_vd(vadd_vd_vd_vd(r.x, r.y), de), (0.).as_vd(), ret);

  ret = vmulsign_vd_vd_vd(ret, x);

  ret = vsel_vd_vo_vd_vd(vlt_vo_vd_vd(nu, de), x, ret);
  ret = vsel_vd_vo_vd_vd(veq_vo_vd_vd(de, (0.).as_vd()), (SLEEF_NAN).as_vd(), ret);
...
/* TODO AArch64: potential optimization by using `vfmad_lane_f64` */
static CONST dd2 gammak(a: VDouble) {
  VDouble2 clc = vcast_vd2_d_d(0, 0), clln = vcast_vd2_d_d(1, 0), clld = vcast_vd2_d_d(1, 0);
  VDouble2 v = vcast_vd2_d_d(1, 0), x, y, z;
  VDouble t, u;

  VOpMask otiny = vlt_vo_vd_vd(vabs_vd_vd(a), (1e-306).as_vd()), oref = vlt_vo_vd_vd(a, (0.5).as_vd());

  x = vsel_vd2_vo_vd2_vd2(otiny, vcast_vd2_d_d(0, 0),
			  vsel_vd2_vo_vd2_vd2(oref, ddadd2_vd2_vd_vd((1.).as_vd(), vneg_vd_vd(a)),
					      vcast_vd2_vd_vd(a, (0.).as_vd())));

  VOpMask o0 = vand_vo_vo_vo(vle_vo_vd_vd((0.5).as_vd(), x.x), vle_vo_vd_vd(x.x, (1.1).as_vd()));
  VOpMask o2 = vle_vo_vd_vd((2.3).as_vd(), x.x);
  ...
  VOpMask o = vand_vo_vo_vo(o2, vle_vo_vd_vd(x.x, (7.).as_vd()));
  ...
  if (!vtestallones_i_vo64(vnot_vo64_vo64(oref))) {
    t = vsub_vd_vd_vd(a, vmul_vd_vd_vd(D1_28, vcast_vd_vi(vtruncate_vi_vd(vmul_vd_vd_vd(a, (1. / D1_28).as_vd())))));
    x = ddmul_vd2_vd2_vd2(clld, sinpik(t));
  }
  
  clld = vsel_vd2_vo_vd2_vd2(otiny, vcast_vd2_vd_vd(vmul_vd_vd_vd(a, (D1_60*D1_60).as_vd()), (0.).as_vd()),
...
pub fn xtgamma_u1(a: VDouble) -> VDouble {
  dd2 d = gammak(a);
  VDouble2 y = ddmul_vd2_vd2_vd2(expk2(d.a), d.b);
  VDouble r = vadd_vd_vd_vd(y.x, y.y);
  VOpMask o;

  o = vor_vo_vo_vo(vor_vo_vo_vo(veq_vo_vd_vd(a, (-SLEEF_INFINITY).as_vd()),
				vand_vo_vo_vo(vlt_vo_vd_vd(a, (0.).as_vd()), visint_vo_vd(a))),
		   vand_vo_vo_vo(vand_vo_vo_vo(visnumber_vo_vd(a), vlt_vo_vd_vd(a, (0.).as_vd())), visnan_vo_vd(r)));
  r = vsel_vd_vo_vd_vd(o, (SLEEF_NAN).as_vd(), r);

  o = vand_vo_vo_vo(vand_vo_vo_vo(vor_vo_vo_vo(veq_vo_vd_vd(a, SLEEF_INFINITY.as_vd()), visnumber_vo_vd(a)),
				  vge_vo_vd_vd(a, (-f64::MIN).as_vd())),
		    vor_vo_vo_vo(vor_vo_vo_vo(veq_vo_vd_vd(a, (0.).as_vd()), vgt_vo_vd_vd(a, (200).as_vd())), visnan_vo_vd(r)));
  r = vsel_vd_vo_vd_vd(o, vmulsign_vd_vd_vd(SLEEF_INFINITY.as_vd(), a), r);
  
  return r;
}

pub fn xlgamma_u1(a: VDouble) -> VDouble {
  dd2 d = gammak(a);
  ...
  o = vor_vo_vo_vo(visinf_vo_vd(a),
		   vor_vo_vo_vo(vand_vo_vo_vo(vle_vo_vd_vd(a, (0.).as_vd()), visint_vo_vd(a)),
...
/* TODO AArch64: potential optimization by using `vfmad_lane_f64` */
pub fn xerf_u1(a: VDouble) -> VDouble {
  VDouble s = a, t, u;
  VDouble2 d;

  a = vabs_vd_vd(a);
  VOpMask o0 = vlt_vo_vd_vd(a, (1.).as_vd());
  VOpMask o1 = vlt_vo_vd_vd(a, (3.7).as_vd());
  VOpMask o2 = vlt_vo_vd_vd(a, (6.).as_vd());
  ...
  u = vmulsign_vd_vd_vd(vsel_vd_vo_vd_vd(o2, vadd_vd_vd_vd(d.x, d.y), (1.).as_vd()), s);
...
/* TODO AArch64: potential optimization by using `vfmad_lane_f64` */
pub fn xerfc_u15(a: VDouble) -> VDouble {
  VDouble s = a, r = (0.).as_vd(), t;
  VDouble2 u, d, x;
  a = vabs_vd_vd(a);
  VOpMask o0 = vlt_vo_vd_vd(a, (1.).as_vd());
  VOpMask o1 = vlt_vo_vd_vd(a, (2.2).as_vd());
  VOpMask o2 = vlt_vo_vd_vd(a, (4.2).as_vd());
  VOpMask o3 = vlt_vo_vd_vd(a, (27.3).as_vd());
  ...
  t = vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.6801072401395386139e-20, 0.3438010341362585303e-12, -0.5757819536420710449e+2, 0.2334249729638701319e+5)
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.2161766247570055669e-18, -0.1237021188160598264e-10, 0.4669289654498104483e+3, -0.4695661044933107769e+5))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.4695919173301595670e-17, 0.2117985839877627852e-09, -0.1796329879461355858e+4, 0.3173403108748643353e+5))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.9049140419888007122e-16, -0.2290560929177369506e-08, 0.4355892193699575728e+4, 0.3242982786959573787e+4))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1634018903557410728e-14, 0.1748931621698149538e-07, -0.7456258884965764992e+4, -0.2014717999760347811e+5))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.2783485786333451745e-13, -0.9956602606623249195e-07, 0.9553977358167021521e+4, 0.1554006970967118286e+5))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.4463221276786415752e-12, 0.4330010240640327080e-06, -0.9470019905444229153e+4, -0.6150874190563554293e+4))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.6711366622850136563e-11, -0.1435050600991763331e-05, 0.7387344321849855078e+4, 0.1240047765634815732e+4))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.9422759050232662223e-10, 0.3460139479650695662e-05, -0.4557713054166382790e+4, -0.8210325475752699731e+2))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.1229055530100229098e-08, -0.4988908180632898173e-05, 0.2207866967354055305e+4, 0.3242443880839930870e+2))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1480719281585086512e-07, -0.1308775976326352012e-05, -0.8217975658621754746e+3, -0.2923418863833160586e+2))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.1636584469123399803e-06, 0.2825086540850310103e-04, 0.2268659483507917400e+3, 0.3457461732814383071e+0))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1646211436588923575e-05, -0.6393913713069986071e-04, -0.4633361260318560682e+2, 0.5489730155952392998e+1))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.1492565035840623511e-04, -0.2566436514695078926e-04, 0.9557380123733945965e+1, 0.1559934132251294134e-2))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1205533298178967851e-03, 0.5895792375659440364e-03, -0.2958429331939661289e+1, -0.1541741566831520638e+1))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.8548327023450850081e-03, -0.1695715579163588598e-02, 0.1670329508092765480e+0, 0.2823152230558364186e-5))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.5223977625442187932e-02, 0.2089116434918055149e-03, 0.6096615680115419211e+0, 0.6249999184195342838e+0))
      .mla(u.x, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.2686617064513125222e-01, 0.1912855949584917753e-01, 0.1059212443193543585e-2, 0.1741749416408701288e-8));
  ...
  x = ddmul_vd2_vd2_vd(vsel_vd2_vo_vd2_vd2(o1, d, vcast_vd2_vd_vd(vneg_vd_vd(a), (0.).as_vd())), a);
  ...
  r = vsel_vd_vo_vd_vd(o3, vadd_vd_vd_vd(x.x, x.y), (0.).as_vd());
  r = vsel_vd_vo_vd_vd(vsignbit_vo_vd(s), vsub_vd_vd_vd((2.).as_vd(), r), r);
...
pub fn VDouble __acos_finite     (VDouble)          __attribute__((weak, alias(str_xacos     )));
pub fn VDouble __acosh_finite    (VDouble)          __attribute__((weak, alias(str_xacosh    )));
pub fn VDouble __asin_finite     (double)           __attribute__((weak, alias(str_xasin_u1  )));
pub fn VDouble __atan2_finite    (VDouble, VDouble) __attribute__((weak, alias(str_xatan2_u1 )));
pub fn VDouble __atanh_finite    (VDouble)          __attribute__((weak, alias(str_xatanh    )));
pub fn VDouble __cosh_finite     (VDouble)          __attribute__((weak, alias(str_xcosh     )));
pub fn VDouble __exp10_finite    (VDouble)          __attribute__((weak, alias(str_xexp10    )));
pub fn VDouble __exp2_finite     (VDouble)          __attribute__((weak, alias(str_xexp2     )));
pub fn VDouble __exp_finite      (VDouble)          __attribute__((weak, alias(str_xexp      )));
pub fn VDouble __fmod_finite     (VDouble, VDouble) __attribute__((weak, alias(str_xfmod     )));
pub fn VDouble __modf_finite     (VDouble, VDouble *) __attribute__((weak, alias(str_xmodf   )));
pub fn VDouble __hypot_u05_finite(VDouble, VDouble) __attribute__((weak, alias(str_xhypot_u05)));
pub fn VDouble __lgamma_u1_finite(VDouble)          __attribute__((weak, alias(str_xlgamma_u1)));
pub fn VDouble __log10_finite    (VDouble)          __attribute__((weak, alias(str_xlog10    )));
pub fn VDouble __log_finite      (VDouble)          __attribute__((weak, alias(str_xlog_u1   )));
pub fn VDouble __pow_finite      (VDouble, VDouble) __attribute__((weak, alias(str_xpow      )));
pub fn VDouble __sinh_finite     (VDouble)          __attribute__((weak, alias(str_xsinh     )));
pub fn VDouble __sqrt_finite     (VDouble)          __attribute__((weak, alias(str_xsqrt     )));
pub fn VDouble __tgamma_u1_finite(VDouble)          __attribute__((weak, alias(str_xtgamma_u1)));
