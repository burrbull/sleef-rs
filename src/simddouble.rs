...
fn vsignbit_vo_vd(d: $f64x) -> $ox {
  ($u64x::from_bits(d) & $u64x::from_bits($f64x::splat(-0.))).eq($u64x::from_bits($f64x::splat(-0.)))
}

// return d0 < d1 ? x : y
#[inline]
fn vsel_vi_vd_vd_vi_vi($f64x d0, $f64x d1, $ix x, $ix y) -> $ix { return $m32x::from_cast(d0.lt(d1)).select(x, y); } 

// return d0 < 0 ? x : 0
#[inline]
fn vsel_vi_vd_vi($f64x d, $ix x) -> $ix { return vand_vi_vo_vi($m32x::from_cast(vsignbit_vo_vd(d)), x); }
#[inline]
...
fn vcopysign_vd_vd_vd(x: $f64x, y: $f64x) -> $f64x {
  $f64x::from_bits(vandnot_vm_vm_vm($u64x::from_bits($f64x::splat(-0.)), $u64x::from_bits(x)) ^
...
fn vilogbk_vi_vd(d: $f64x) -> $ix {
  $u64x o = d.lt($f64x::splat(4.9090934652977266e-91));
  d = o.select($f64x::splat(2.037035976334486e90) * d, d);
  let q = $ix::splat($i64x::from_cast(d));
...
#endif
#[inline]
fn visint_vo_vd(d: $f64x) -> $u64x {
  $f64x x = (d * $f64x::splat(1. / D1_31)).truncate();
  x = $f64x::splat(-D1_31).mul_add(x, d);
  x.truncate().eq(x) | d.abs().gt($f64x::splat(D1_53))
}
#[inline]
fn visodd_vo_vd(d: $f64x) -> $u64x {
  $f64x x = (d * $f64x::splat(1. / D1_31)).truncate();
  x = $f64x::splat(-D1_31).mul_add(x, d);
...
pub fn xilogb(d: $f64x) -> $ix {
  $f64x e = $f64x::from_cast(vilogbk_vi_vd(d.abs()));
  e = d.eq($f64x::splat(0.)).select($f64x::splat(SLEEF_FP_ILOGB0), e);
...
fn rempisub(x: $f64x) -> ($f64x, $ix) {
   if cfg!(feature="full_fp_rounding") {
    $f64x y = (x * $f64x::splat(4.)).rint();
    $ix vi = (y - x.rint() * $f64x::splat(4.))..truncatei();
    ( x - y * $f64x::splat(0.25), vi )
   } else {
    $f64x fr = x - $f64x::splat(D1_28) * (x * $f64x::splat(1. / D1_28)).truncate();
    $ix vi = $m32x::from_cast(x.gt($f64x::splat(0.))).select($ix::splat(4), $ix::splat(3)) + (fr * $f64x::splat(8.)).truncatei();
    vi = (($ix::splat(7) & vi) - $ix::splat(3)) >> 1;
    fr = fr - $f64x::splat(0.25) * fr.mul_add($f64x::splat(4.), vmulsign_vd_vd_vd($f64x::splat(0.5), x)).truncate();
    fr = fr.abs().gt($f64x::splat(0.25)).select(fr - vmulsign_vd_vd_vd($f64x::splat(0.5), x), fr);
    fr = fr.abs().gt($f64x::splat(1e+10)).select($f64x::splat(0), fr);
    $u64x o = x.abs().eq($f64x::splat(0.12499999999999998612));
    fr = o.select(x, fr);
...
fn rempi(a: $f64x) -> (D2, $ix) {
...
#endif
  ex = ex - $ix::splat(55);
  $ix q = vand_vi_vo_vi(ex.gt($ix::splat(700-55)), $ix::splat(-64));
  ...
  $u64x o = a.abs().lt($f64x::splat(0.7));
  x.0 = o.select(a, x.0);
...
pub fn xsin(d: $f64x) -> $f64x {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    ...
    ql = ddii & $ix::splat(3);
    ql = ql + ql + $m32x::from_cast(ddidd.0.gt($f64x::splat(0.))).select($ix::splat(2), $ix::splat(1));
    ql = ql >> 2;
    $u64x o = (ddii & $ix::splat(1)).eq($ix::splat(1));
    ...
    d = ddidd.0 + ddidd.1;
    d = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(r) | visnan_vo_vd(r), $u64x::from_bits(d)));
  }

  s = d * d;

  d = $f64x::from_bits(vand_vm_vo64_vm($mx::from_cast((ql & $ix::splat(1)).eq($ix::splat(1))), $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(d));
  ...
  u = visnegzero_vo_vd(r).select(r, u);
...
pub fn xsin_u1(d: $f64x) -> $f64x {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    const $f64x dql = (d*$f64x::splat(M_1_PI)).rint();
    ql = dql.rinti();
    u = dql.mul_add($f64x::splat(-PI_A2), d);
    s = u.add_checked_as_d2(dql*$f64x::splat(-PI_B2));
  } else if d.abs().lt($f64x::splat(TRIGRANGEMAX)).all() {
    $f64x dqh = (d*$f64x::splat(M_1_PI / D1_24)).truncate();
    dqh = dqh*$f64x::splat(D1_24);
    const $f64x dql = (d.mul_sub($f64x::splat(M_1_PI), dqh)).rint();
    ql = dql.rinti();

    u = dqh.mul_add($f64x::splat(-PI_A), d);
    ...
    ddidd = vsel_vd2_vo_vd2_vd2($mx::from_cast(o), x, ddidd);
    s = ddidd.normalize();
    s.0 = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(d) | visnan_vo_vd(d), $u64x::from_bits(s.0)));
  }
  ...
  u = $f64x::from_bits(vand_vm_vo64_vm($mx::from_cast((ql & $ix::splat(1)).eq($ix::splat(1))),
						       $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(u));
  u = d.eq($f64x::splat(0.)).select(d, u);
...
pub fn xcos(d: $f64x) -> $f64x {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    ...
    $f64x dqh = d.mul_add($f64x::splat(M_1_PI / D1_23), $f64x::splat(-M_1_PI / D1_24)).truncate();
    ql = (d*$f64x::splat(M_1_PI) +
				   dqh.mul_add($f64x::splat(-D1_23), $f64x::splat(-0.5))).rinti();
    dqh = dqh*$f64x::splat(D1_24);
    ql = ql + ql + $ix::splat(1);
    $f64x dql = $f64x::from_cast(ql);
    ...
    let (ddidd, ddii) = rempi(d);
    ql = ddii & $ix::splat(3);
    ql = ql + ql + $m32x::from_cast(ddidd.0.gt($f64x::splat(0.))).select($ix::splat(8), $ix::splat(7));
    ql = ql >> 1;
    $u64x o = (ddii & $ix::splat(1)).eq($ix::splat(0));
    $f64x y = ddidd.0.gt($f64x::splat(0.)).select($f64x::splat(0.), $f64x::splat(-1.));
  ...
  d = $f64x::from_bits(vand_vm_vo64_vm($mx::from_cast((ql & $ix::splat(2)).eq($ix::splat(0))), $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(d));
...
pub fn xcos_u1(d: $f64x) -> $f64x {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    $f64x dql = d.mul_add($f64x::splat(M_1_PI), $f64x::splat(-0.5)).rint();
    ...
    dqh = dqh*$f64x::splat(D1_24);
    ql = ql + ql + $ix::splat(1);
    const $f64x dql = $f64x::from_cast(ql);
  ...
  u = $f64x::from_bits(vand_vm_vo64_vm($mx::from_cast((ql & $ix::splat(2)).eq($ix::splat(0))), $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(u));
...
pub fn xsincos(d: $f64x) -> ($f64x, $f64x) {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    $f64x dql = (d*$f64x::splat(2 * M_1_PI)).rint();
    ql = dql.rinti();
    s = dql.mul_add($f64x::splat(-PI_A2 * 0.5), d);
    s = dql.mul_add($f64x::splat(-PI_B2 * 0.5), s);
  } else if d.abs().lt($f64x::splat(TRIGRANGEMAX)).all() {
    $f64x dqh = (d*$f64x::splat(2*M_1_PI / D1_24)).truncate();
    dqh = dqh*$f64x::splat(D1_24);
    $f64x dql = (d*$f64x::splat(2*M_1_PI) - dqh).rint();
    ql = dql.rinti();
    ...
    ql = ddii;
    s = ddidd.0 + ddidd.1;
    s = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(d) | visnan_vo_vd(d), $u64x::from_bits(s)));
  }
  ...
  rx = (u*s).mul_add(t, t);
  rx = visnegzero_vo_vd(d).select($f64x::splat(-0.), rx);
  ...
  ry = s.mul_add(u, $f64x::splat(1.));

  o = $mx::from_cast(i(ql & $ix::splat(1)).eq($ix::splat(0)));
  rsin = o.select(rx, ry);
  rcos = o.select(ry, rx);

  o = $mx::from_cast((ql & $ix::splat(2)).eq($ix::splat(2)));
  rsin = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(rsin));

  o = $mx::from_cast(((ql + $ix::splat(1)) & $ix::splat(2)).eq($ix::splat(2)));
  rcos = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(rcos));
...
pub fn xsincos_u1(d: $f64x) -> ($f64x, $f64x) {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    const $f64x dql = (d*$f64x::splat(2 * M_1_PI)).rint();
    ql = dql.rinti();
    u = dql.mul_add($f64x::splat(-PI_A2*0.5), d);
    s = u.add_checked_as_d2(dql*$f64x::splat(-PI_B2*0.5));
  } else if d.abs().lt($f64x::splat(TRIGRANGEMAX)).all() {
    $f64x dqh = (d*$f64x::splat(2*M_1_PI / D1_24)).truncate();
    dqh = dqh*$f64x::splat(D1_24);
    const $f64x dql = (d*$f64x::splat(2*M_1_PI) - dqh).rint();
    ql = dql.rinti();
  ...
  o = $mx::from_cast((ql & $ix::splat(1)).eq($ix::splat(0)));
  rsin = o.select(rx, ry);
  rcos = o.select(ry, rx);
...
pub fn xsincospi_u05(d: $f64x) -> ($f64x, $f64x) {
  ...
  o = $mx::from_cast((q & $ix::splat(2)).eq($ix::splat(0)));
  rsin = o.select(rx, ry);
  rcos = o.select(ry, rx);

  o = $mx::from_cast((q & $ix::splat(4)).eq($ix::splat(4)));
  rsin = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(rsin));

  o = $mx::from_cast(((q + $ix::splat(2)) & $ix::splat(4)).eq($ix::splat(4)));
  rcos = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(rcos));

  o = d.abs().gt($f64x::splat(TRIGRANGEMAX3/4.));
...
pub fn xmodf(x: $f64x) -> ($f64x, $f64x) {
  $f64x fr = x - $f64x::splat(D1_31)*$f64x::from_cast((x*$f64x::splat(1. / D1_31)).truncatei());
...
fn sinpik(d: $f64x) -> D2 {
  ...
  $ix q = u.truncatei();
  q = (q + ($ix::from_bits($u64x::from_bits(q) >> 31) ^ $ix::splat(1))) & $ix::splat(!1);
  o = $mx::from_cast((q & $ix::splat(2)).eq($ix::splat(2)));
  ...
  u = vsel_vd_vo_d_d(o, 9.94480387626843774090208e-16, -2.02461120785182399295868e-14)
      .mul_add(s, vsel_vd_vo_d_d(o, -3.89796226062932799164047e-13, 6.948218305801794613277840e-12))
      .mul_add(s, vsel_vd_vo_d_d(o, 1.150115825399960352669010e-10, -1.75724749952853179952664e-09))
      .mul_add(s, vsel_vd_vo_d_d(o, -2.46113695010446974953590e-08, 3.133616889668683928784220e-07))
      .mul_add(s, vsel_vd_vo_d_d(o, 3.590860448590527540050620e-06, -3.65762041821615519203610e-05))
      .mul_add(s, vsel_vd_vo_d_d(o, -0.000325991886927389905997954, 0.0024903945701927185027435600));
  ...
  o = $mx::from_cast((q & $ix::splat(4)).eq($ix::splat(4)));
  x.0 = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(x.0));
  x.1 = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(x.1));
...
pub fn xsinpi_u05(d: $f64x) -> $f64x {
  ...
  r = $f64x::from_bits(vandnot_vm_vo64_vm(d.abs().gt($f64x::splat(TRIGRANGEMAX3/4.)), $u64x::from_bits(r)));
  r = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(d), $u64x::from_bits(r)));
...
pub fn xcospi_u05(d: $f64x) -> $f64x {
  ...
  r = d.abs().gt($f64x::splat(TRIGRANGEMAX3/4.)).select($f64x::splat(1.), r);
  r = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(d), $u64x::from_bits(r)));
...
pub fn xtan(d: $f64x) -> $f64x {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    ...
    x = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(d) | visnan_vo_vd(d), $u64x::from_bits(x)));
  }

  s = x*x;

  o = $mx::from_cast((ql & $ix::splat(1)).eq($ix::splat(1)));
  x = $f64x::from_bits(vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(x));
...
pub fn xtan_u1(d: $f64x) -> $f64x {
  ...
  if d.abs().lt($f64x::splat(TRIGRANGEMAX2)).all() {
    ...
    s = D2::from((M_2_PI_H, M_2_PI_L)) * d +
			  (d.lt($f64x::splat(0.)).select(
							 $f64x::splat(-0.5), $f64x::splat(0.5)) - dqh);
    const $f64x dql = (s.0 + s.1).truncate();
    ql = dql.rinti();
  ...
  o = $mx::from_cast((ql & $ix::splat(1)).eq($ix::splat(1)));
  vmask n = vand_vm_vo64_vm(o, $u64x::from_bits($f64x::splat(-0.)));
...
fn atan2k(y: $f64x, x: $f64x) -> $f64x {
  ...
  t = s.mul_add(t*u, s);
  t = $f64x::from_cast(q).mul_add($f64x::splat(M_PI/2.), t);
...
fn atan2k_u1(D2 y, D2 x) -> D2 {
  ...
  vmask b = vand_vm_vo64_vm(p, $u64x::from_bits($f64x::splat(-0.)));
  ...
  t = (D2::from((1.570796326794896557998982, 6.12323399573676603586882e-17)) * $f64x::from_cast(q)).add_checked(t);

  return t;
}
#[inline]
fn visinf2_vd_vd_vd(d: $f64x, m: $f64x) -> $f64x {
  return $f64x::from_bits(vand_vm_vo64_vm(visinf_vo_vd(d), ($u64x::from_bits(d) & $u64x::from_bits($f64x::splat(-0.))) | $u64x::from_bits(m))));
}

pub fn xatan2(y: $f64x, x: $f64x) -> $f64x {
  $f64x r = atan2k(y.abs(), x);

  r = vmulsign_vd_vd_vd(r, x);
  r = (visinf_vo_vd(x) | x.eq($f64x::splat(0.))).select($f64x::splat(M_PI/2.) - visinf2_vd_vd_vd(x, vmulsign_vd_vd_vd($f64x::splat(M_PI/2.), x)), r);
  r = visinf_vo_vd(y).select($f64x::splat(M_PI/2.) - visinf2_vd_vd_vd(x, vmulsign_vd_vd_vd($f64x::splat(M_PI/4.), x)), r);
  r = y.eq($f64x::splat(0.)).select($f64x::from_bits(vand_vm_vo64_vm(vsignbit_vo_vd(x), $u64x::from_bits($f64x::splat(M_PI)))), r);

  r = $f64x::from_bits(vor_vm_vo64_vm(visnan_vo_vd(x) | visnan_vo_vd(y), $u64x::from_bits(vmulsign_vd_vd_vd(r, y))));
  return r;
}

pub fn xatan2_u1(y: $f64x, x: $f64x) -> $f64x {
  $u64x o = x.abs().lt($f64x::splat(5.5626846462680083984e-309)); // nexttoward((1.0 / DBL_MAX), 1)
  x = o.select(x*$f64x::splat(D1_53), x);
  y = o.select(y*$f64x::splat(D1_23), y);

  D2 d = atan2k_u1(D2::new(y.abs(), $f64x::splat(0.)), D2::new(x, $f64x::splat(0.)));
  $f64x r = d.0 + d.1;
...
pub fn xasin(d: $f64x) -> $f64x {
  $u64x o = d.abs().lt($f64x::splat(0.5));
  $f64x x2 = o.select(d*d, ($f64x::splat(1.) - d.abs())*$f64x::splat(0.5));
  $f64x x = o.select(d.abs(), x2.sqrt());
  ...
  $f64x r = o.select(u, u.mul_add($f64x::splat(-2.), $f64x::splat(M_PI/2.)));
...
pub fn xasin_u1(d: $f64x) -> $f64x {
  ...
  $f64x x2 = o.select(d*d, ($f64x::splat(1.)-d.abs())*$f64x::splat(0.5));
  u;
  D2 x = vsel_vd2_vo_vd2_vd2(o, D2::new(d.abs(), $f64x::splat(0.)), x2.sqrt_as_d2());
...
pub fn xacos(d: $f64x) -> $f64x {
  $u64x o = d.abs().lt($f64x::splat(0.5));
  $f64x x2 = o.select(d*d,
				($f64x::splat(1.) - d.abs())*$f64x::splat(0.5)), u;
  $f64x x = o.select(d.abs(), x2.sqrt());
  ...
  x = x + u;
  $f64x r = o.select(y, x*$f64x::splat(2.));
...
pub fn xacos_u1(d: $f64x) -> $f64x {
  $u64x o = d.abs().lt($f64x::splat(0.5));
  $f64x x2 = o.select(d*d, ($f64x::splat(1.) - d.abs())*$f64x::splat(0.5)), u;
  D2 x = vsel_vd2_vo_vd2_vd2(o, D2::new(d.abs(), $f64x::splat(0.)), x2.sqrt_as_d2());
  x = vsel_vd2_vo_vd2_vd2(d.abs().eq($f64x::splat(1.)), D2::from((0., 0.)), x);
...
pub fn xatan_u1(d: $f64x) -> $f64x {
  D2 d2 = atan2k_u1(D2::new(d.abs(), $f64x::splat(0.)), D2::from((1., 0.)));
  $f64x r = d2.0 + d2.1;
...
pub fn xatan(s: $f64x) -> $f64x {
  ...
  t = $mx::from_cast((q & $ix::splat(1)).eq($ix::splat(1))).select($f64x::splat(M_PI/2.) - t, t);
  t = $f64x::from_bits(vand_vm_vo64_vm($mx::from_cast((q & $ix::splat(2)).eq($ix::splat(2))), $u64x::from_bits($f64x::splat(-0.))) ^ $u64x::from_bits(t));
...
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  $u64x o = d.lt($f64x::splat(f64::MIN));
...
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  x = x.mul_add(t, $f64x::splat(0.693147180559945286226764)*$f64x::from_cast(e));

  x = vispinf_vo_vd(d).select($f64x::splat(SLEEF_INFINITY), x);
  x = (d.lt($f64x::splat(0.)) | visnan_vo_vd(d)).select($f64x::splat(SLEEF_NAN), x);
  x = d.eq($f64x::splat(0.)).select($f64x::splat(-SLEEF_INFINITY), x);
#else
  x = x.mul_add(t, $f64x::splat(0.693147180559945286226764)*e);
  x = vfixup_vd_vd_vd_vi2_i(x, d, $i64x::splat((5 << (5*4))), 0);
#endif
...
pub fn xexp(d: $f64x) -> $f64x {
  $f64x u = (d*$f64x::splat(R_LN2)).rint();
  s;
  $ix q = u.rinti();
  ...
  u = d.gt($f64x::splat(709.78271114955742909217217426)).select($f64x::splat(SLEEF_INFINITY), u);
  u = $f64x::from_bits(vandnot_vm_vo64_vm(d.lt($f64x::splat(-1000.)), $u64x::from_bits(u)));
...
fn expm1k(d: $f64x) -> $f64x {
  ...
  u = $mx::from_cast(q.eq($ix::splat(0))).select(u,
...
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  s = D2::from((0.693147180559945286226764, 2.319046813846299558417771e-17)) * $f64x::from_cast(e);
#else
...
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  D2 s = D2::from((0.693147180559945286226764, 2.319046813846299558417771e-17)) * $f64x::from_cast(e);
#else
...
#if !defined(ENABLE_AVX512F) && !defined(ENABLE_AVX512FNOFMA)
  r = vispinf_vo_vd(d).select($f64x::splat(SLEEF_INFINITY), r);
  r = (d.lt($f64x::splat(0.)) | visnan_vo_vd(d)).select($f64x::splat(SLEEF_NAN), r);
  r = d.eq($f64x::splat(0.)).select($f64x::splat(-SLEEF_INFINITY), r);
#else
  r = vfixup_vd_vd_vd_vi2_i(r, d, $i64x::splat((4 << (2*4)) | (3 << (4*4)) | (5 << (5*4)) | (2 << (6*4))), 0);
#endif
...
fn expk(D2 d) -> $f64x {
  ...
  u = $f64x::from_bits(vandnot_vm_vo64_vm(d.0.lt($f64x::splat(-1000.)), $u64x::from_bits(u)));
...
#if 1
  $u64x yisint = visint_vo_vd(y);
  $u64x yisodd = visodd_vo_vd(y) & yisint;

  D2 d = logk(x.abs()) * y;
  $f64x result = expk(d);
  result = d.0.gt($f64x::splat(709.78271114955742909217217426)).select($f64x::splat(SLEEF_INFINITY), result);

  result *= x.gt($f64x::splat(0.)).select(
					  $f64x::splat(1.),
					  yisint.select(yisodd.select($f64x::splat(-1.), $f64x::splat(1.)), $f64x::splat(SLEEF_NAN)));

  $f64x efx = vmulsign_vd_vd_vd(x.abs() - $f64x::splat(1.), y);

  result = visinf_vo_vd(y).select(
			    $f64x::from_bits(vandnot_vm_vo64_vm(efx.lt($f64x::splat(0.)),
								  $u64x::from_bits(efx.eq($f64x::splat(0.)).select(
												      $f64x::splat(1.),
												      $f64x::splat(SLEEF_INFINITY))))),
			    result);

  result = (visinf_vo_vd(x) | x.eq($f64x::splat(0.))).select(
			    yisodd.select(vsign_vd_vd(x), $f64x::splat(1.)) *
					  $f64x::from_bits(vandnot_vm_vo64_vm(x.eq($f64x::splat(0.)).select(-y, y).lt($f64x::splat(0.)),
										$u64x::from_bits($f64x::splat(SLEEF_INFINITY)))),
			    result);

  result = $f64x::from_bits(vor_vm_vo64_vm(visnan_vo_vd(x) | visnan_vo_vd(y), $u64x::from_bits(result)));

  result = (y.eq($f64x::splat(0.)) | x.eq($f64x::splat(1.))).select($f64x::splat(1.), result);
...
fn expk2(D2 d) -> D2 {
  ...
  t.0 = $f64x::from_bits(vandnot_vm_vo64_vm(d.0.lt($f64x::splat(-1000.)), $u64x::from_bits(t.0)));
  t.1 = $f64x::from_bits(vandnot_vm_vo64_vm(d.0.lt($f64x::splat(-1000.)), $u64x::from_bits(t.1)));
...
pub fn xsinh(x: $f64x) -> $f64x {
  ...
  y = (x.abs().gt($f64x::splat(710.)) | visnan_vo_vd(y)).select($f64x::splat(SLEEF_INFINITY), y);
...
pub fn xtanh(x: $f64x) -> $f64x {
  ...
  y = (x.abs().gt($f64x::splat(18.714973875)) | visnan_vo_vd(y)).select($f64x::splat(1.), y);
  y = vmulsign_vd_vd_vd(y, x);
  y = $f64x::from_bits(vor_vm_vo64_vm(visnan_vo_vd(x), $u64x::from_bits(y)));
...
pub fn xsinh_u35(x: $f64x) -> $f64x {
  ...
  y = (x.abs().gt($f64x::splat(709.)) | visnan_vo_vd(y)).select($f64x::splat(SLEEF_INFINITY), y);
  y = vmulsign_vd_vd_vd(y, x);
  y = $f64x::from_bits(vor_vm_vo64_vm(visnan_vo_vd(x), $u64x::from_bits(y)));
...
pub fn xcosh_u35(x: $f64x) -> $f64x {
  $f64x e = xexp(x.abs());
  $f64x y = $f64x::splat(0.5).mul_add(e, $f64x::splat(0.5) / e);
...
pub fn xtanh_u35(x: $f64x) -> $f64x {
  $f64x d = expm1k($f64x::splat(2.)*x.abs());
...
fn logk2(D2 d) -> D2 {
  ...
  m.0 = vldexp2_vd_vd_vi(d.0, -e);
  m.1 = vldexp2_vd_vd_vi(d.1, -e);
...
pub fn xasinh(x: $f64x) -> $f64x {
  $f64x y = x.abs();
  $u64x o = y.gt($f64x::splat(1.));
  D2 d;
  
  d = vsel_vd2_vo_vd2_vd2(o, x.recpre_as_d2(), D2::new(y, $f64x::splat(0.)));
  ...
  y = (x.abs().gt($f64x::splat(SQRT_DBL_MAX)) | visnan_vo_vd(y)).select(
...
pub fn xacosh(x: $f64x) -> $f64x {
  ...
  y = $f64x::from_bits(vandnot_vm_vo64_vm(x.eq($f64x::splat(1.)), $u64x::from_bits(y)));
...
pub fn xatanh(x: $f64x) -> $f64x {
  ...
  y = $f64x::from_bits(vor_vm_vo64_vm(y.gt($f64x::splat(1.)), $u64x::from_bits(y.eq($f64x::splat(1.)).select($f64x::splat(SLEEF_INFINITY), (d.0 + d.1)*$f64x::splat(0.5)))));

  y = vmulsign_vd_vd_vd(y, x);
  y = $f64x::from_bits(vor_vm_vo64_vm(visinf_vo_vd(x) | visnan_vo_vd(y), $u64x::from_bits(y)));
  y = $f64x::from_bits(vor_vm_vo64_vm(visnan_vo_vd(x), $u64x::from_bits(y)));
...
pub fn xcbrt(d: $f64x) -> $f64x {
  ...
  $f64x s = d;
#endif
  e = vilogbk_vi_vd(d.abs()) + $ix::splat(1);
  d = vldexp2_vd_vd_vi(d, -e);

  t = $f64x::from_cast(e) + $f64x::splat(6144.);
  qu = (t*$f64x::splat(1./3.)).truncatei();
  re = (t*($f64x::from_cast(qu)*$f64x::splat(3.))).truncatei();

  q = $mx::from_cast(re.eq($ix::splat(1))).select($f64x::splat(1.2599210498948731647672106), q);
  q = $mx::from_cast(re.eq($ix::splat(2))).select($f64x::splat(1.5874010519681994747517056), q);
  ...
  if cfg!(feature = "enable_avx512f") || cfg!(feature = "enable_avx512fnofma") {
    ...
    y = s.eq($f64x::splat(0.)).select(vmulsign_vd_vd_vd($f64x::splat(0.), s), y);
  }
...
pub fn xcbrt_u1(d: $f64x) -> $f64x {
  ...
  q2 = vsel_vd2_vo_vd2_vd2($mx::from_cast(re.eq($ix::splat(1))), D2::from((1.2599210498948731907, -2.5899333753005069177e-17)), q2);
  q2 = vsel_vd2_vo_vd2_vd2($mx::from_cast(re.eq($ix::splat(2))), D2::from((1.5874010519681995834, -1.0869008194197822986e-16)), q2);
  ...
  if !cfg!(feature = "enable_avx512f") && !cfg!(feature = "enable_avx512fnofma") {
    z = visinf_vo_vd(d).select(vmulsign_vd_vd_vd($f64x::splat(SLEEF_INFINITY), q2.0), z);
    d.eq($f64x::splat(0.)).select($f64x::from_bits(vsignbit_vm_vd(q2.0)), z)
  } else {
    z = visinf_vo_vd(s).select(vmulsign_vd_vd_vd($f64x::splat(SLEEF_INFINITY), s), z);
    s.eq($f64x::splat(0.)).select(vmulsign_vd_vd_vd($f64x::splat(0.), s), z)
  }
}
...
pub fn xexp2(d: $f64x) -> $f64x {
  ...
  u = d.ge($f64x::splat(1024.)).select($f64x::splat(SLEEF_INFINITY), u);
  $f64x::from_bits(vandnot_vm_vo64_vm(d.lt($f64x::splat(-2000.)), $u64x::from_bits(u)))
}
...
pub fn xexp10(d: $f64x) -> $f64x {
  ...
  u = d.gt($f64x::splat(308.25471555991671)).select($f64x::splat(SLEEF_INFINITY), u);
  $f64x::from_bits(vandnot_vm_vo64_vm(d.lt($f64x::splat(-350.)), $u64x::from_bits(u)))
}

pub fn xexpm1(a: $f64x) -> $f64x {
  let d = expk2(D2::new(a, $f64x::splat(0.))) + $f64x::splat(-1.);
  let mut x = d.0 + d.1;
  x = a.gt($f64x::splat(709.782712893383996732223)).select($f64x::splat(SLEEF_INFINITY), x);
  x = a.lt($f64x::splat(-36.736800569677101399113302437)).select($f64x::splat(-1.), x);
...
pub fn xlog10(d: $f64x) -> $f64x {
  ...
  if !cfg!(feature = "enable_avx512f") && !cfg!(feature = "enable_avx512fnofma") {
      ...
      let r = (d.lt($f64x::splat(0.)) | visnan_vo_vd(d)).select($f64x::splat(SLEEF_NAN), r);
      d.eq($f64x::splat(0.)).select($f64x::splat(-SLEEF_INFINITY), r)
  } else {
      vfixup_vd_vd_vd_vi2_i(r, d, $i64x::splat((4 << (2*4)) | (3 << (4*4)) | (5 << (5*4)) | (2 << (6*4))), 0)
  }
}
...
pub fn xlog2(d: $f64x) -> $f64x {
  ...
  let mut s = if !cfg!(feature = "enable_avx512f") && !cfg!(feature = "enable_avx512fnofma") {
      ...
      m = vldexp3_vd_vd_vi(d, -e);
      e = $m32x::from_cast(o).select(e - $ix::splat(64), e);
      $f64x::from_cast(e) + x * D2::from((2.885390081777926774, 6.0561604995516736434e-18))
  } else {
...
pub fn xlog1p(d: $f64x) -> $f64x {
  ...
  let mut s = if !cfg!(feature = "enable_avx512f") && !cfg!(feature = "enable_avx512fnofma") {
      ...
      e = $m32x::from_cast(o).select(e - $ix::splat(64), e);
      D2::from((0.693147180559945286226764, 2.319046813846299558417771e-17)) * $f64x::from_cast(e)
    } else {
  ...
  r = d.gt($f64x::splat(1e+307)).select($f64x::splat(SLEEF_INFINITY), r);
  r = (d.lt($f64x::splat(-1.)) | visnan_vo_vd(d)).select($f64x::splat(SLEEF_NAN), r);
  r = d.eq($f64x::splat(-1.)).select($f64x::splat(-SLEEF_INFINITY), r);
  visnegzero_vo_vd(d).select($f64x::splat(-0.), r)
}

//
#[inline]
fn vcast_vi2_i_i(i0: u32, i1: u32) -> $i64x { $i64x::from_bits($u64x::from_u32((i0, i1))) }

pub fn xfabs(x: $f64x) -> $f64x { x.abs() }

pub fn xcopysign(x: $f64x, y: $f64x) -> $f64x { vcopysign_vd_vd_vd(x, y) }
...
pub fn xtrunc(x: $f64x) -> $f64x {
  let mut fr = x - $f64x::splat(D1_31)*$f64x::from_cast((x*$f64x::splat(1. / D1_31)).truncatei());
  fr -= $f64x::from_cast(fr.truncatei());
  (visinf_vo_vd(x) | x.abs().ge($f64x::splat(D1_52))).select(x, vcopysign_vd_vd_vd(x - fr, x))
}
...
pub fn xround(d: $f64x) -> $f64x {
  ...
  fr = fr.lt($f64x::splat(0.)).select(fr + $f64x::splat(1.), fr);
  x = d.eq($f64x::splat(0.49999999999999994449)).select($f64x::splat(0.), x);
  (visinf_vo_vd(d) | d.abs().ge($f64x::splat(D1_52))).select(d, vcopysign_vd_vd_vd(x - fr, d))
}
...
pub fn xrint(d: $f64x) -> $f64x {
  ...
  fr -= $f64x::from_cast(fr.truncatei());
  fr = (fr.lt($f64x::splat(0)) | (fr.eq($f64x::splat(0)) & isodd)).select(fr + $f64x::splat(1.), fr);
  x = d.eq($f64x::splat(0.50000000000000011102)).select($f64x::splat(0.), x);
  (visinf_vo_vd(d) | d.abs().ge($f64x::splat(D1_52)).select(d, vcopysign_vd_vd_vd(x - fr, d))
}

pub fn xnextafter(x: $f64x, y: $f64x) -> $f64x {
  let x = x.eq($f64x::splat(0.)).select(vmulsign_vd_vd_vd($f64x::splat(0.), y), x);
  ...
  xi2 = $i64x::from_cast(x.ne(y).select(
					     $f64x::from_bits(xi2 + vrev21_vi2_vi2(vcast_vi2_i_i(0, -1) & veq_vi2_vi2_vi2(xi2, vcast_vi2_i_i(0, -1)))),
					     $f64x::from_bits(xi2)));
  ...
  ret = (ret.eq($f64x::splat(0.)) & x.ne($f64x::splat(0.))).select( 
...
pub fn xfrfrexp(x: $f64x) -> $f64x {
  let x = x.abs().lt($f64x::splat(f64::MIN)).select(x*$f64x::splat(D1_63), x);
  ...
  let ret = visinf_vo_vd(x).select(vmulsign_vd_vd_vd($f64x::splat(SLEEF_INFINITY), x), ret);
...
pub fn xexpfrexp(x: $f64x) -> $ix {
  ...
  ret = ($ix::from_bits($u64x::from_bits(ret) >> 20) & $ix::splat(0x7ff)) - $ix::splat(0x3fe);
...
fn xsqrt_u05(d: $f64x) -> $f64x {
  ...
  let q = o.select($f64x::splat(2.9387358770557188e-39*0.5), $f64x::splat(0.5));
  ...
  let mut x = $f64x::from_bits(vcast_vi2_i_i(0x5fe6ec86, 0) - $i64x::from_bits($u64x::from_cast(d + $f64x::splat(1e-320)) >> 1));
...
pub fn xhypot_u05(x: $f64x, y: $f64x) -> $f64x {
  ...
  (x.eq($f64x::splat(SLEEF_INFINITY)) | y.eq($f64x::splat(SLEEF_INFINITY))).select($f64x::splat(SLEEF_INFINITY), ret)
}
...
fn vptrunc(x: $f64x) -> $f64x {
  let mut fr = $f64x::splat(-D1_31).mul_add($f64x::from_cast((x*$f64x::splat(1. / D1_31)).truncatei()), x);
  fr -= $f64x::from_cast(fr.truncatei());
...
pub fn xfmod(x: $f64x, y: $f64x) -> $f64x {
  ...
  for _ in 0..21 { // ceil(log2(DBL_MAX) / 51) + 1
    let q = ((de + de).gt(r.0) & r.0.ge(de)).select(
			 $f64x::splat(1.), vtoward0(r.0)*rde);
    let q = $f64x::from_bits($u64x::from_bits(vptrunc(q)) & $u64x::from_u32((0xffffffff, 0xfffffffe)));
    r = (r + q.mul_as_d2(-de)).normalize();
    if r.0.lt(de).all() { break; }
  }
...
fn gammak(a: $f64x) -> (D2<$f64x, $f64x) {
  ...
  let x = vsel_vd2_vo_vd2_vd2(otiny, D2::from((0., 0.)),
  ...
  let u = vsel_vd_vo_vo_d_d_d(o2, o0, -156.801412704022726379848862, 0.2947916772827614196e+2, 0.7074816000864609279e-7)
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 1.120804464289911606838558160000, 0.1281459691827820109e+3, 0.4009244333008730443e-6))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 13.39798545514258921833306020000, 0.2617544025784515043e+3, 0.1040114641628246946e-5))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.116546276599463200848033357000, 0.3287022855685790432e+3, 0.1508349150733329167e-5))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -1.391801093265337481495562410000, 0.2818145867730348186e+3, 0.1288143074933901020e-5))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.015056113040026424412918973400, 0.1728670414673559605e+3, 0.4744167749884993937e-6))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.179540117061234856098844714000, 0.7748735764030416817e+2, -0.6554816306542489902e-7))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.002481743600264997730942489280, 0.2512856643080930752e+2, -0.3189252471452599844e-6))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.029527880945699120504851034100, 0.5766792106140076868e+1, 0.1358883821470355377e-6))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.000540164767892604515196325186, 0.7270275473996180571e+0, -0.4343931277157336040e-6))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.006403362833808069794787256200, 0.8396709124579147809e-1, 0.9724785897406779555e-6))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.000162516262783915816896611252, -0.8211558669746804595e-1, -0.2036886057225966011e-5))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.001914438498565477526465972390, 0.6828831828341884458e-1, 0.4373363141819725815e-5))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 7.20489541602001055898311517e-05, -0.7712481339961671511e-1, -0.9439951268304008677e-5))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.000839498720672087279971000786, 0.8337492023017314957e-1, 0.2050727030376389804e-4))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -5.17179090826059219329394422e-05, -0.9094964931456242518e-1, -0.4492620183431184018e-4))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.000592166437353693882857342347, 0.1000996313575929358e+0, 0.9945751236071875931e-4))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 6.97281375836585777403743539e-05, -0.1113342861544207724e+0, -0.2231547599034983196e-3))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.000784039221720066627493314301, 0.1255096673213020875e+0, 0.5096695247101967622e-3))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.000229472093621399176949318732, -0.1440498967843054368e+0, -0.1192753911667886971e-2))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, -0.002681327160493827160473958490, 0.1695571770041949811e+0, 0.2890510330742210310e-2))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.003472222222222222222175164840, -0.2073855510284092762e+0, -0.7385551028674461858e-2))
      .mul_add(t, vsel_vd_vo_vo_d_d_d(o2, o0, 0.083333333333333333335592087900, 0.2705808084277815939e+0, 0.2058080842778455335e-1));

  let mut y = (x + $f64x::splat(-0.5)) * logk2(x);
  y += -x;
  y += D2::from((0.91893853320467278056, -3.8782941580672414498e-17)); // 0.5*log(2*M_PI)

  let mut z = u.mul_as_d2(t) + vsel_vd_vo_d_d(o0, -0.4006856343865314862e+0, -0.6735230105319810201e-1);
...
pub fn xerf_u1(a: $f64x) -> $f64x {
  ...
  let t = vsel_vd_vo_vo_d_d_d(o0, o1, 0.6801072401395392157e-20, 0.2830954522087717660e-13, -0.5846750404269610493e-17)
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.2161766247570056391e-18, -0.1509491946179481940e-11, 0.6076691048812607898e-15))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.4695919173301598752e-17, 0.3827857177807173152e-10, -0.3007518609604893831e-13))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.9049140419888010819e-16, -0.6139733921558987241e-09, 0.9427906260824646063e-12))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.1634018903557411517e-14, 0.6985387934608038824e-08, -0.2100110908269393629e-10))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.2783485786333455216e-13, -0.5988224513034371474e-07, 0.3534639523461223473e-09))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.4463221276786412722e-12, 0.4005716952355346640e-06, -0.4664967728285395926e-08))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.6711366622850138987e-11, -0.2132190104575784400e-05, 0.4943823283769000532e-07))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.9422759050232658346e-10, 0.9092461304042630325e-05, -0.4271203394761148254e-06))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.1229055530100228477e-08, -0.3079188080966205457e-04, 0.3034067677404915895e-05))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.1480719281585085023e-07, 0.7971413443082370762e-04, -0.1776295289066871135e-04))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.1636584469123402714e-06, -0.1387853215225442864e-03, 0.8524547630559505050e-04))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.1646211436588923363e-05, 0.6469678026257590965e-04, -0.3290582944961784398e-03))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.1492565035840624866e-04, 0.4996645280372945860e-03, 0.9696966068789101157e-03))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.1205533298178966496e-03, -0.1622802482842520535e-02, -0.1812527628046986137e-02))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.8548327023450851166e-03, 0.1615320557049377171e-03, -0.4725409828123619017e-03))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.5223977625442188799e-02, 0.1915262325574875607e-01, 0.2090315427924229266e-01))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.2686617064513125569e-01, -0.1027818298486033455e+00, -0.1052041921842776645e+00))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, 0.1128379167095512753e+00, -0.6366172819842503827e+00, -0.6345351808766568347e+00))
      .mul_add(u, vsel_vd_vo_vo_d_d_d(o0, o1, -0.3761263890318375380e+00, -0.1128379590648910469e+01, -0.1129442929103524396e+01));
...
pub fn xerfc_u15(a: $f64x) -> $f64x {
  ...
  let u = vsel_vd2_vo_vd2_vd2(o0, a.mul_as_d2(a), vsel_vd2_vo_vd2_vd2(o1, D2::new(a, $f64x::splat(0.)), D2::from((1., 0.)) / D2::new(a, $f64x::splat(0.))));

  let t = vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.6801072401395386139e-20, 0.3438010341362585303e-12, -0.5757819536420710449e+2, 0.2334249729638701319e+5)
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.2161766247570055669e-18, -0.1237021188160598264e-10, 0.4669289654498104483e+3, -0.4695661044933107769e+5))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.4695919173301595670e-17, 0.2117985839877627852e-09, -0.1796329879461355858e+4, 0.3173403108748643353e+5))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.9049140419888007122e-16, -0.2290560929177369506e-08, 0.4355892193699575728e+4, 0.3242982786959573787e+4))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1634018903557410728e-14, 0.1748931621698149538e-07, -0.7456258884965764992e+4, -0.2014717999760347811e+5))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.2783485786333451745e-13, -0.9956602606623249195e-07, 0.9553977358167021521e+4, 0.1554006970967118286e+5))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.4463221276786415752e-12, 0.4330010240640327080e-06, -0.9470019905444229153e+4, -0.6150874190563554293e+4))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.6711366622850136563e-11, -0.1435050600991763331e-05, 0.7387344321849855078e+4, 0.1240047765634815732e+4))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.9422759050232662223e-10, 0.3460139479650695662e-05, -0.4557713054166382790e+4, -0.8210325475752699731e+2))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.1229055530100229098e-08, -0.4988908180632898173e-05, 0.2207866967354055305e+4, 0.3242443880839930870e+2))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1480719281585086512e-07, -0.1308775976326352012e-05, -0.8217975658621754746e+3, -0.2923418863833160586e+2))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.1636584469123399803e-06, 0.2825086540850310103e-04, 0.2268659483507917400e+3, 0.3457461732814383071e+0))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1646211436588923575e-05, -0.6393913713069986071e-04, -0.4633361260318560682e+2, 0.5489730155952392998e+1))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.1492565035840623511e-04, -0.2566436514695078926e-04, 0.9557380123733945965e+1, 0.1559934132251294134e-2))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.1205533298178967851e-03, 0.5895792375659440364e-03, -0.2958429331939661289e+1, -0.1541741566831520638e+1))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.8548327023450850081e-03, -0.1695715579163588598e-02, 0.1670329508092765480e+0, 0.2823152230558364186e-5))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, 0.5223977625442187932e-02, 0.2089116434918055149e-03, 0.6096615680115419211e+0, 0.6249999184195342838e+0))
      .mul_add(u.0, vsel_vd_vo_vo_vo_d_d_d_d(o0, o1, o2, -0.2686617064513125222e-01, 0.1912855949584917753e-01, 0.1059212443193543585e-2, 0.1741749416408701288e-8));
