...
#[inline]
fn vandnot_vm_vm_vm(x: $mx, y: $mx) -> $mx { return $mx::from(_mm_andnot_pd($f64x::from(x), $f64x::from(y))); }

#[inline]
fn vandnot_vo_vo_vo(x: $mox, y: $mox) -> $mox { return $mx::from(_mm_andnot_pd($f64x::from(x), $f64x::from(y))); }

#[inline]
fn vand_vm_vo64_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_and_pd($f64x::from(x), $f64x::from(y))); }
#[inline]
fn vandnot_vm_vo64_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_andnot_pd($f64x::from(x), $f64x::from(y))); }
#[inline]
fn vor_vm_vo64_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_or_pd($f64x::from(x), $f64x::from(y))); }
#[inline]
fn vxor_vm_vo64_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_xor_pd($f64x::from(x), $f64x::from(y))); }

#[inline]
fn vand_vm_vo32_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_and_pd($f64x::from(x), $f64x::from(y))); }
#[inline]
fn vandnot_vm_vo32_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_andnot_pd($f64x::from(x), $f64x::from(y))); }
#[inline]
fn vor_vm_vo32_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_or_pd($f64x::from(x), $f64x::from(y))); }
#[inline]
fn vxor_vm_vo32_vm(x: $mox, y: $mx) -> $mx { return $mx::from(_mm_xor_pd($f64x::from(x), $f64x::from(y))); }
...
fn visinf_vo_vd(d: $f64x) -> $mox {
  return $mx::from(_mm_cmp_pd(d.abs(), _mm_set1_pd(SLEEF_INFINITY), _CMP_EQ_OQ));
}

#[inline]
fn vispinf_vo_vd(d: $f64x) -> $mox {
  return $mx::from(_mm_cmp_pd(d, _mm_set1_pd(SLEEF_INFINITY), _CMP_EQ_OQ));
}
...
fn visnan_vo_vd(d: $f64x) -> $mox {
  return $mx::from(_mm_cmp_pd(d, d, _CMP_NEQ_UQ));
}
...
#[inline]
fn vrint_vi2_vf(vf: $f32x) -> $ix2 { return $ix2::from(_mm_cvtps_epi32(vf)); }
#[inline]
fn vtruncate_vi2_vf(vf: $f32x) -> $ix2 { return $ix2::from(_mm_cvttps_epi32(vf)); }
impl Rec for $f32x {
    #[inline]
    fn rec(self) -> Self {
        Self::splat(1.) / self
